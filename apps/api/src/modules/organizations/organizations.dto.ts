...
export function toOrganizationDtos(rows: OrganizationRow[]): OrganizationDto[] {
  return rows.map(toOrganizationDto);
}

// ---------------------------------------------------------------------------
// Normalize input: resolve frontend name strings → DB-ready snake_case params.
// ---------------------------------------------------------------------------
...
export function normalizeUpdateInput(
  ...
  resolvedRepId: number | null,
  resolvedDirectorId: number | null,
): ResolvedUpdateParams {
  const params: ResolvedUpdateParams = {};
  if (input.name !== undefined) params.name = input.name.trim();
  if (input.state !== undefined) params.state = input.state.trim().toUpperCase();
  ...
  if (input.territoryId !== undefined) params.territory_id = input.territoryId;
  ...
  if (input.teamColors !== undefined) params.team_colors = input.teamColors;
  if (input.region !== undefined) params.region = input.region;
  if (input.stateMarket !== undefined) params.state_market = input.stateMarket;
  if (input.division !== undefined) params.division = input.division;
  if (input.territory !== undefined) params.territory = input.territory;
  if (input.subterritory !== undefined) params.subterritory = input.subterritory;
  if (input.sportFocus !== undefined) params.sport_focus = input.sportFocus;
  return params;
}
