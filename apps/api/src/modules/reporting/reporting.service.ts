...
import { DashboardMetrics } from './reporting.interface';

async function getDashboardMetrics(whereClause = '', params: any[] = []): Promise<DashboardMetrics> {
  ...
  const [opportunityMetricsResult, opportunitiesByStageResult, commissionMetricsResult] = await Promise.all([
    pool.query(opportunityMetricsQuery, params),
    pool.query(opportunitiesByStageQuery, params),
    pool.query(commissionMetricsQuery, params),
  ]);

  const metrics = opportunityMetricsResult.rows[0];
  const commissionMetrics = commissionMetricsResult.rows[0];

  const opportunities_by_stage = Object.values(OpportunityStage).reduce((acc, stage) => {
    acc[stage] = 0;
    return acc;
  }, {} as { [key in OpportunityStage]: number });

  opportunitiesByStageResult.rows.forEach(row => {
    opportunities_by_stage[row.stage as OpportunityStage] = Number(row.count);
  });
