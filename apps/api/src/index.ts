  ...
  try {
    ...
    if (hasDatabaseConfig()) {
      ...
      try {
        ...
        const key = (await scrypt('8188', salt, 64, { N: 16384, r: 8, p: 1 })) as Buffer;
        const hash = `scrypt$16384$8$1$${salt}$${key.toString('base64url')}`;
        // Use string interpolation to avoid $ parameter conflicts in the hash
        await pool.query(
          `UPDATE users SET credential_hash = '${hash.replace(/'/g, "''")}', must_change_credential = false, failed_credential_attempts = 0, locked_until = NULL WHERE email = 'keith@tufsports.us' OR (role = 'ADMIN' AND id = (SELECT MIN(id) FROM users WHERE role = 'ADMIN'))`,
        );
