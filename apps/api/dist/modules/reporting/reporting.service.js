...
function directorScope(directorId, stateMarket) {
    const states = stateMarket ? stateMarket.split(',').map((s) => s.trim()).filter(Boolean) : [];
    if (states.length === 0) {
...
async function getDashboardMetrics(scope) {
    ...
    `, [...params, AUDITABLE_TOUCH_TYPES]),
      ...
      ${orgScope}
    `, params),
        database_1.pool.query(`SELECT o.stage, COUNT(*)::int AS count FROM opportunities o JOIN organizations org ON org.id = o.organization_id ${orgScope} GROUP BY o.stage`, params),
        database_1.pool.query(`
      SELECT
...
async function getDirectorDashboardMetrics(directorId) {
    ...
    const userResult = await database_1.pool.query('SELECT state_market FROM users WHERE id = $1 AND role = $2', [directorId, 'DIRECTOR']);
