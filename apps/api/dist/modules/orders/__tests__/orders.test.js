...
describe('Orders Service - Integration Test', () => {
    ...
    const createTestOrgAndOpps = async () => {
        ...
        await (0, opportunities_service_1.updateOpportunityStage)(oppToWin.id, opportunities_interface_1.OpportunityStage.CONTACTED, 1);
        await (0, opportunities_service_1.updateOpportunityStage)(oppToWin.id, opportunities_interface_1.OpportunityStage.DISCOVERY, 1);
        await (0, opportunities_service_1.updateOpportunityStage)(oppToWin.id, opportunities_interface_1.OpportunityStage.MOCKUP_REQUESTED, 1);
        await (0, opportunities_service_1.updateOpportunityStage)(oppToWin.id, opportunities_interface_1.OpportunityStage.MOCKUP_DELIVERED, 1);
        ...
        await (0, opportunities_service_1.updateOpportunityStage)(oppToWin.id, opportunities_interface_1.OpportunityStage.DECISION_PENDING, 1);
    ...
    it('should auto-create an order when opportunity moves to CLOSED_WON', async () => {
        const { closedWonOpp } = await createTestOrgAndOpps();
        const order = await (0, orders_service_1.getOrderByOpportunityId)(closedWonOpp.id);
    ...
    it('should reject duplicate order creation for the same opportunity', async () => {
        const { closedWonOpp } = await createTestOrgAndOpps();
        const firstOrder = await (0, orders_service_1.getOrderByOpportunityId)(closedWonOpp.id);
    ...
    it('should return an order by id after creation', async () => {
        const { closedWonOpp } = await createTestOrgAndOpps();
        const createdOrder = await (0, orders_service_1.getOrderByOpportunityId)(closedWonOpp.id);
