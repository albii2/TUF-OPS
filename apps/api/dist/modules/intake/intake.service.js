...
exports.CLASSIFICATION_TYPES = void 0;
exports.computeAttentionScore = computeAttentionScore;
exports.classifyIntake = classifyIntake;
exports.createIntakeItem = createIntakeItem;
exports.getIntakeItems = getIntakeItems;
exports.getLighthouseView = getLighthouseView;
exports.recalculateAttentionScores = recalculateAttentionScores;
...
exports.createStatusCheckBatch = createStatusCheckBatch;
exports.getStatusCheckSummary = getStatusCheckSummary;
const database_1 = require("@packages/database");
// ── Classification Engine ──
/** Classification types matching the Meridian OS spec */
exports.CLASSIFICATION_TYPES = [
 ...
 */
function computeAttentionScore(item) {
    let score = 0;
    // Base priority score
    ...
    const urgentTypes = ['revenue_blocker', 'vendor_delayed', 'rep_inactive', 'status_check_response'];
    ...
    if (item.due_date) {
        ...
        if (dueInHours < 0)
            ...
            score += 20; // Due within 24h
        else if (dueInHours < 72)
 ...
 */
function classifyIntake(item) {
    const text = `${item.title} ${item.description || ''} ${(item.tags || []).join(' ')}`.toLowerCase();
    const src = (item.source || '').toLowerCase();
    if (src === 'recruiting' || text.includes('applicant') || text.includes('candidate') || text.includes('hire')) {
        if (text.includes('offer') || text.includes('accepted') || text.includes('acceptance'))
            return 'recruiting_offer_accepted';
        return 'recruiting_application';
    }
    if (src === 'hr' || text.includes('onboarding') || text.includes('hr') || text.includes('personnel'))
        return 'hr_onboarding';
    if (text.includes('mockup') || text.includes('design') || text.includes('creative'))
        return 'mockup_requested';
    if (text.includes('vendor') || text.includes('delayed') || text.includes('production'))
        return 'vendor_delayed';
    if (text.includes('order') || text.includes('paid') || text.includes('payment'))
        return 'order_paid';
    if (text.includes('inactive') || text.includes('attrition') || text.includes('not active'))
        return 'rep_inactive';
    if (text.includes('coach') || text.includes('reply') || text.includes('responded'))
        return 'coach_reply';
    if (text.includes('meeting') || text.includes('call') || text.includes('appointment'))
        return 'meeting_completed';
    if (text.includes('territory') || text.includes('assigned') || text.includes('reassign'))
        return 'territory_assigned';
    if (text.includes('revenue') || text.includes('blocker') || text.includes('bottleneck') || text.includes('stuck'))
        return 'revenue_blocker';
    if (text.includes('pipeline') || text.includes('forecast') || text.includes('opportunity'))
        return 'pipeline_update';
    if (text.includes('certification') || text.includes('academy') || text.includes('training'))
        return 'academy_certification';
    if (text.includes('decision') || text.includes('approval') || text.includes('executive'))
        return 'executive_decision';
    if (text.includes('status check') || text.includes('check-in') || text.includes('checkin') || text.includes('leadership status'))
 ...
 */
async function getLighthouseView() {
...
async function recalculateAttentionScores() {
    const items = await getIntakeItems({ status: 'open' });
    let updated = 0;
    for (const item of items) {
        const newScore = computeAttentionScore(item);
        if (newScore !== (item.attention_score || 0)) {
            await database_1.pool.query('UPDATE executive_intake SET attention_score = $1 WHERE id = $2', [newScore, item.id]);
            updated++;
        }
    }
    return updated;
 ...
 */
async function createStatusCheckBatch(data) {
    ...
    try {
        ...
        for (const recipient of data.recipients) {
            ...
            const result = await client.query(`INSERT INTO executive_intake 
 ...
 */
async function getStatusCheckSummary() {
    const result = await database_1.pool.query(`SELECT * FROM executive_intake WHERE classification_type = 'status_check_response' 
