...
async function intakeRoutes(server) {
    ...
    server.get('/lighthouse', intake_controller_1.lighthouseHandler);
    // POST /api/intake/status-check — create batch status check
    server.post('/status-check', intake_controller_1.createStatusCheckHandler);
    // GET /api/intake/status-check — get response summary
    server.get('/status-check', intake_controller_1.getStatusCheckHandler);
