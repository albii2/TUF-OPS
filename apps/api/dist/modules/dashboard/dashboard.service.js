...
async function getExecutiveDashboard() {
    const empty = { rows: [] };
    const [decisions, hr, overdue, pipelineStats, revenueBlockers, recruiting, sales, ops, waiting, dailyBrief] = await Promise.all([
        safeQuery(() => database_1.pool.query(`SELECT * FROM executive_intake WHERE status = 'open' AND (priority = 'critical' OR priority = 'high')
       ORDER BY CASE priority WHEN 'critical' THEN 0 ELSE 1 END, created_at DESC LIMIT 10`), empty),
        safeQuery(() => database_1.pool.query(`SELECT candidate_name, current_stage, assigned_hr, notes FROM people_pipeline
       WHERE status = 'active' AND current_stage NOT IN ('first_order', 'first_proposal')
       ORDER BY created_at DESC LIMIT 10`), empty),
        safeQuery(() => database_1.pool.query(`SELECT * FROM executive_intake WHERE status = 'open' AND created_at < NOW() - INTERVAL '7 days'
       ORDER BY created_at ASC LIMIT 10`), empty),
        safeQuery(() => database_1.pool.query(`SELECT current_stage, COUNT(*) as count FROM people_pipeline WHERE status = 'active' GROUP BY current_stage`), empty),
        safeQuery(() => database_1.pool.query(`SELECT * FROM executive_intake WHERE status = 'open' AND 'revenue' = ANY(tags) ORDER BY priority LIMIT 5`), empty),
        safeQuery(() => database_1.pool.query(`SELECT c.first_name || ' ' || c.last_name AS name, c.stage, c.assigned_director_id AS owner_id,
       ...
       WHERE c.stage NOT IN ('rejected', 'hired') ORDER BY c.updated_at DESC NULLS LAST LIMIT 10`), empty),
        safeQuery(() => database_1.pool.query(`SELECT o.id, o.name AS title, o.stage, o.assigned_rep_id, o.value, o.next_action, o.expected_close_date
       ...
       ORDER BY o.expected_close_date ASC NULLS LAST LIMIT 10`), empty),
        safeQuery(() => database_1.pool.query(`SELECT ord.id, ord.status, ord.assigned_rep_id, o.name AS opportunity_name, ord.updated_at
       ...
       ORDER BY ord.updated_at ASC NULLS LAST LIMIT 10`), empty),
        safeQuery(() => database_1.pool.query(`SELECT * FROM executive_intake WHERE status = 'open' AND (owner IS NULL OR owner = '')
       ORDER BY created_at DESC LIMIT 10`), empty),
        safeQuery(() => database_1.pool.query(`SELECT
