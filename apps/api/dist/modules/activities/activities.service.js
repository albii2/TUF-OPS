...
async function getActivitiesByOrganization(organizationId) {
    const result = await database_1.pool.query('SELECT * FROM activities WHERE organization_id = $1 ORDER BY created_at DESC', [organizationId]);
    return result.rows;
}
async function getAllActivities(limit = 100) {
    const result = await database_1.pool.query('SELECT * FROM activities ORDER BY created_at DESC LIMIT $1', [limit]);
    return result.rows;
}
