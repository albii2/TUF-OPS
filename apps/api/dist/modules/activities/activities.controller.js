...
exports.listActivitiesHandler = listActivitiesHandler;
...
async function listActivitiesHandler(request, reply) {
    const { entityType, entityId, limit } = request.query;
    if (entityType === 'ORGANIZATION' && entityId) {
        const activities = await (0, activities_service_1.getActivitiesByOrganization)(Number(entityId));
        return reply.send(activities.slice(0, limit ? Number(limit) : undefined));
    }
    if (entityType === 'OPPORTUNITY' && entityId) {
        const activities = await (0, activities_service_1.getActivitiesByOpportunity)(Number(entityId));
        return reply.send(activities.slice(0, limit ? Number(limit) : undefined));
    }
    // No entityType specified — return all recent activities
    if (!entityType && !entityId) {
        const activities = await (0, activities_service_1.getAllActivities)(limit ? Number(limit) : 100);
        return reply.send(activities);
    }
