...
export async function getForgeMission() {
    ...
    const oppMap = {};
    for (const o of opps) {
        if (!oppMap[o.organizationId])
            oppMap[o.organizationId] = [];
    ...
    for (const org of orgs) {
        ...
        if (best && best.stage === 'INVOICE_SENT') {
            score = 80;
            reason = 'Invoice sent — close the order.';
            action = 'Call coach to confirm order and submit.';
            stage = best.stage;
        }
        else if (best && best.stage === 'MOCKUP_STAGE') {
            score = 60;
            reason = 'Mockup in progress — advance quickly.';
            action = 'Follow up on mockup approval.';
            stage = best.stage;
        }
        else if (best && best.stage === 'DISCOVERY') {
            score = 40;
            reason = 'Discovery stage — deepen the conversation.';
            action = 'Schedule product presentation.';
            stage = best.stage;
        }
        else if (best && best.stage === 'LEAD_ENGAGED') {
            score = 25;
            reason = 'Initial contact made — build the relationship.';
            action = 'Send lookbook and schedule call.';
            stage = best.stage;
        }
        else if (days > 14) {
            score = 15;
            reason = `No contact in ${days} days — re-engage.`;
            action = 'Make contact via phone or email.';
        }
        else if (orgOpps.length === 0) {
            score = 5;
            reason = 'No opportunities — open a lane.';
            action = 'Identify sport/program need and propose.';
        }
        else {
            score = 10;
            reason = 'Low activity — maintain relationship.';
            action = 'Send seasonal update or catalog.';
        }
        // Add sport season urgency bonus
        const sport = best?.sport || '';
        if (sport.toLowerCase().includes('football'))
            score += 5;
        else if (sport.toLowerCase().includes('volleyball'))
            score += 4;
