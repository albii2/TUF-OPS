...
export function getDirectorRepSet(directorName) {
    return new Set(getManagedRepNamesForDirector(directorName));
}
export function getDirectorTerritorySet(directorName) {
    return new Set(getManagedTerritoriesForDirector(directorName));
}
export function canViewOrganization(org) {
    ...
    if (!user || user.role === 'ADMIN' || user.role === 'REGIONAL_DIRECTOR' || user.role === 'OPERATIONS')
        return true;
    if (user.role === 'REP')
        return org.assignedRep === user.name;
...
export function canViewOpportunity(opp) {
    const user = getViewer();
    if (!user || user.role === 'ADMIN' || user.role === 'REGIONAL_DIRECTOR' || user.role === 'OPERATIONS')
        return true;
    if (user.role === 'REP')
        return opp.assignedRep === user.name;
    if (user.role === 'DIRECTOR') {
        const reps = getDirectorRepSet(user.name);
...
export function canViewOrder(order, linkedOpportunity) {
    const user = getViewer();
    if (!user || user.role === 'ADMIN' || user.role === 'REGIONAL_DIRECTOR' || user.role === 'OPERATIONS')
        return true;
    ...
    if (user.role === 'REP')
        return orderRep === user.name;
    if (user.role === 'DIRECTOR')
        return orderDirector === user.name || (!!orderRep && getDirectorRepSet(user.name).has(orderRep));
    return false;
}
export function isRepCertified(user) {
    if (!user)
        return false;
...
export function canCreateOpportunity() {
    ...
    if (user.role === 'REP')
        return isRepCertified(user) || isSandboxActive;
    return false;
}
export function canAdvanceOpportunity(opp) {
    const user = getViewer();
    if (!user)
        return false;
    const isSandboxActive = typeof window !== 'undefined' && localStorage.getItem('tuf_combine_sandbox_active') === 'true';
    if (!isRepCertified(user) && !isSandboxActive)
        return false;
    if (user.role === 'ADMIN')
        return true;
    if (user.role === 'REP')
...
export function getAdvanceDeniedMessage(opp) {
    const user = getViewer();
    if (!user)
        return 'Log in before advancing an opportunity.';
    const isSandboxActive = typeof window !== 'undefined' && localStorage.getItem('tuf_combine_sandbox_active') === 'true';
    if (!isRepCertified(user) && !isSandboxActive)
        return 'Onboarding and certification is required before you can perform sales actions.';
    if (user.role === 'DIRECTOR')
        return opp.assignedRep === user.name ? '' : 'You can only advance opportunities assigned to you.';
    if (user.role === 'REP' && opp.assignedRep !== user.name)
        return 'You can only advance opportunities assigned to you.';
