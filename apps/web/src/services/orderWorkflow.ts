...
export const ORDER_STAGE_FLOW: OrderStage[] = [
  'ORDER_CREATED',
  'PAYMENT_CONFIRMED',
  'ARTWORK_FINALIZED',
  'VENDOR_READY',
...
const STATUS_STAGE_MAP: Record<Order['productionStatus'], OrderStage> = {
...
const STAGE_STATUS_MAP: Record<OrderStage, Order['productionStatus']> = {
...
const TRANSITION_FIELDS: Record<string, AdvancementField[]> = {
  ORDER_CREATED: [
...
export const BLOCKER_FIELDS: AdvancementField[] = [
...
export function getOrderStageLabel(stageOrOrder: OrderStage | Order) {
  const stage = typeof stageOrOrder === 'string' ? stageOrOrder : getOrderStage(stageOrOrder);
  return ORDER_STAGE_LABELS[stage];
}

export function toProductionStatus(stage: OrderStage): Order['productionStatus'] {
  return STAGE_STATUS_MAP[stage];
}

export function getPreviousActiveStage(order: Order): OrderStage {
  const stage = getOrderStage(order);
  if (stage !== 'BLOCKED_ON_HOLD') return stage;
  return order.previousActiveStage ?? 'VENDOR_READY';
}

export function getNextOrderStage(order: Order): OrderStage | null {
  const stage = getOrderStage(order);
  if (stage === 'BLOCKED_ON_HOLD') return getPreviousActiveStage(order);
  const index = ORDER_STAGE_FLOW.indexOf(stage);
  if (index < 0 || index >= ORDER_STAGE_FLOW.length - 1) return null;
  return ORDER_STAGE_FLOW[index + 1];
}

export function getAdvanceFields(stage: OrderStage) {
  return TRANSITION_FIELDS[stage] ?? [];
}

export function getOrderTitle(order: Order, opportunity?: Opportunity) {
...
export function getOrderOwner(order: Order, opportunity?: Opportunity) {
  return order.nextActionOwner ?? order.assignedRep ?? opportunity?.assignedRep ?? 'Ops';
}

export function getOrderDueDate(order: Order) {
  if (order.dueDate) return order.dueDate;
  const stage = getOrderStage(order);
  if (stage === 'COMPLETED') return order.completedDate ?? order.createdDate;
  if (stage === 'BLOCKED_ON_HOLD') return order.resolutionDueDate ?? addDays(1);
  const age = daysSince(order.createdDate);
  if (age >= 7) return addDays(-1);
  if (stage === 'ORDER_CREATED' || stage === 'PAYMENT_CONFIRMED' || stage === 'ARTWORK_FINALIZED') return addDays(1);
  if (stage === 'VENDOR_READY') return addDays(2);
  return addDays(5);
}
...
export function getOrderRisk(order: Order): { level: OrderRiskLevel; label: string; reason: string; rank: number; tone: string } {
  const stage = getOrderStage(order);
...
export function matchesOrderQueueFilter(order: Order, filter: OrderQueueFilter) {
  const stage = getOrderStage(order);
  const risk = getOrderRisk(order);
  if (filter === 'ALL') return true;
  if (filter === 'ACTION_NEEDED') return stage !== 'COMPLETED' && (risk.level === 'red' || risk.level === 'yellow' || stage !== 'IN_PRODUCTION');
  if (filter === 'IN_PRODUCTION') return stage === 'IN_PRODUCTION' || stage === 'QUALITY_CHECK';
  if (filter === 'BLOCKED') return stage === 'BLOCKED_ON_HOLD' || risk.level === 'red';
  if (filter === 'COMPLETED') return stage === 'COMPLETED';
  return true;
}

export function sortOrdersForExecution(a: Order, b: Order) {
...
export function getOrderAdvanceWarning(order: Order, opportunity?: Opportunity) {
  const user = getViewer();
  if (!user) return 'Log in before updating this order.';
  if (user.role === 'DIRECTOR') return 'You are updating this order as a director. This action will be logged.';
  if (user.role === 'REP' && (order.assignedRep ?? opportunity?.assignedRep) !== user.name) return 'You can only advance orders assigned to you.';
  return '';
}
