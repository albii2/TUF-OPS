...
function normalizeApiOrganization(raw) {
    const zone = raw.tuf_zone || raw.territory || '';
    const territory = ZONE_TO_TERRITORY[zone] || 'metro';
...
export async function listOrganizations(params = {}) {
    ...
    if (params.territory && params.territory !== 'ALL')
        query.territory = params.territory;
    if (params.director && params.director !== 'ALL')
        query.director = params.director;
    if (params.coverageStatus && params.coverageStatus !== 'ALL')
        query.coverageStatus = params.coverageStatus;
    if (params.priority && params.priority !== 'ALL')
        query.priority = params.priority;
    const raw = await apiClient('/organizations', { query });
...
export async function updateOrganization(id, patch) {
    return apiClient(`/organizations/${id}`, { method: 'PUT', body: patch });
}
export async function getOrganizationById(id) {
    return apiClient(`/organizations/${id}`);
}
// Backward-compat stubs — these are no-ops since mock mode is removed
export function listUntouchedAccounts() { return []; }
export function listStaleAccounts() { return []; }
export function listAccountsNeedingAction() { return []; }
