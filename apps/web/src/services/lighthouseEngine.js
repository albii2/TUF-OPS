...
function getSeason(sport) {
    ...
    if (s.includes('football') || s.includes('volleyball') || s.includes('soccer') || s.includes('cross country'))
        return 'Fall';
    if (s.includes('basketball') || s.includes('wrestling') || s.includes('hockey'))
        return 'Winter';
    if (s.includes('baseball') || s.includes('softball') || s.includes('track') || s.includes('golf') || s.includes('tennis'))
        return 'Spring';
    return 'Year-round';
}
function getSeasonUrgency(season) {
    const now = new Date();
    const month = now.getMonth(); // 0=Jan, 6=Jul
    if (season === 'Summer' && (month >= 4 && month <= 7))
        return 'NOW';
    if (season === 'Fall' && (month >= 6 && month <= 10))
        return 'NOW';
    if (season === 'Winter' && (month >= 9 && month <= 12))
        return 'NOW';
    if (season === 'Spring' && (month >= 1 && month <= 4))
        return 'NOW';
    if (season === 'Fall' && month >= 4)
        return 'SOON';
    if (season === 'Winter' && month >= 7)
        return 'SOON';
    if (season === 'Spring' && month >= 10)
        return 'SOON';
    return 'LATER';
}
export async function getOrganizationIntel(orgId) {
    ...
    const revenueLanesMissing = ALL_LANES.filter((l) => !lanesActivated.has(l));
    // Contacts
    ...
    // Buying windows
    const buyingWindows = sportsMapped.map((sport) => {
    ...
    let score = 0;
    if (sportsMapped.length > 0)
        score += 20; // Has mapped sports
    if (sportsMapped.length >= 3)
        score += 15; // Multiple sports
    if (revenueLanesActivated.length > 0)
        score += 20; // Has active lanes
    if (revenueLanesActivated.length >= 2)
        score += 15; // Multiple lanes
    if (activities.length > 0)
        score += 10; // Has activity
    if (activities.length >= 5)
        score += 10; // Significant activity
    if (opps.some((o) => o.stage === 'CLOSED_WON'))
        score += 10; // Has won business
...
//# sourceMappingURL=lighthouseEngine.js.map