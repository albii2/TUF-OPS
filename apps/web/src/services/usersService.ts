...
function readStoredUsers(): StoredManagedUser[] {
  ...
  try {
    ...
    const safeRows = rows.map(({ pin: _pin, ...row }: any) => row) as StoredManagedUser[];
    const localRows = safeRows.filter((row) => !seedIds.has(row.id) && !seedNames.has(row.displayName) && row.id.startsWith('u-local-'));
    const mergedRows = [...seedRows, ...localRows];
    const shouldRewrite = rows.some((row: any) => row.pin)
      || rows.length !== mergedRows.length
      || rows.some((row, index) => mergedRows[index]?.id !== row.id || (seedIds.has(row.id) && mergedRows[index]?.credentialHash !== seedRows.find((seed) => seed.id === row.id)?.credentialHash));
    if (shouldRewrite) {
