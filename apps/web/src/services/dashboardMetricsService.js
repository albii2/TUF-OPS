...
export function emptyDashboardMetrics() {
    return {
        assigned_schools: 0,
        touched_schools: 0,
        untouched_schools: 0,
...
export async function fetchDashboardMetrics(role, userId, userEmail) {
    try {
        if (role === 'ADMIN')
            return apiClient('/reporting/owner-dashboard');
        if (role === 'OPERATIONS')
            return apiClient('/reporting/owner-dashboard');
        if (role === 'DIRECTOR') {
            const id = numericId(userId);
            if (id)
                return apiClient(`/reporting/director-dashboard/${id}`);
            if (userEmail)
                return apiClient('/reporting/director-dashboard-by-email', { query: { email: userEmail } });
            throw new Error('Director dashboard requires backend numeric user id or email in API mode');
        }
        if (role === 'REP') {
            const id = numericId(userId);
            if (id)
                return apiClient(`/reporting/rep-dashboard/${id}`);
            if (userEmail)
                return apiClient('/reporting/rep-dashboard-by-email', { query: { email: userEmail } });
