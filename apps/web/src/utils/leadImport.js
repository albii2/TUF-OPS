...
function normalizeTerritory(value) {
    ...
    if (!territoryKey || territoryKey === 'unassigned')
        return '';
    return TERRITORY_MAP[territoryKey] ?? '';
}
function parseMinnesotaAddress(address) {
    ...
    const stripNonCityLeadTokens = (candidateWords) => {
        const cleaned = [...candidateWords];
        while (cleaned.length && directionals.has(cleanToken(cleaned[0])))
            cleaned.shift();
...
export function parseSportsFromWideColumns(row) {
    const sports = ['football', 'basketball', 'hockey', 'baseball'];
    const offered = [];
    const urls = [];
    sports.forEach((s) => {
        if ((row[`${s}_offered`] ?? '').toLowerCase() === 'yes' || (row[`${s}_offered`] ?? '').toLowerCase() === 'true')
            offered.push(toTitleCase(s));
        if (row[`${s}_urls`])
            urls.push(row[`${s}_urls`]);
    });
    if (row.sports_offered)
        offered.push(...row.sports_offered.split('|').map((x) => toTitleCase(x.trim())).filter(Boolean));
    if (row.sport_urls)
        urls.push(...row.sport_urls.split('|').map((x) => x.trim()).filter(Boolean));
    return { sportsOffered: Array.from(new Set(offered)), sportUrls: urls };
}
export function buildDuplicateKey(row) {
...
export function validateLeadRow(row) {
    ...
    if (row.primaryContactEmail && !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(row.primaryContactEmail))
        errors.push('invalid contact email');
    if (row.headCoachEmail && !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(row.headCoachEmail))
        errors.push('invalid head coach email');
    return errors;
}
export function normalizeLeadRow(rawRow) {
...
//# sourceMappingURL=leadImport.js.map