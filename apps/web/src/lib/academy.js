...
export const LEVEL_1_MODULES = [
    {
        code: 'ACAD-101',
        name: 'The TUF Philosophy',
        description: 'Why TUF exists, the mission, sales expectations, the four-order baseline, lane penetration, and the 7 Sales Philosophy principles.',
        completionCriteria: 'Write a paragraph explaining TUF’s mission in your own words, reviewed by your Director.',
        demonstrateTask: 'Write a paragraph explaining TUF’s mission in your own words and submit it for Director review.',
    ...
    },
    {
        code: 'ACAD-102',
        name: 'Prospecting',
        description: 'How to identify programs, research, make first contact, and qualify. Territory awareness and activity mindset.',
        completionCriteria: 'Create 5+ organizations, log 3+ prospecting activities, and build one week’s pipeline.',
        demonstrateTask: 'Add 5 organizations, log 3 prospecting activities, and build one week’s pipeline.',
    ...
    },
    {
        code: 'ACAD-103',
        name: 'Discovery',
        description: 'How to have a discovery conversation, identify all applicable sales lanes, and record needs correctly.',
        completionCriteria: 'Create an opportunity, identify all applicable sales lanes, and record needs correctly in the opportunity.',
        demonstrateTask: 'Create an opportunity, identify all applicable sales lanes, and record needs correctly.',
    ...
    },
    {
        code: 'ACAD-104',
        name: 'Proposal',
        description: 'How to build a proposal from discovery notes, mockup process, pricing strategy, and presenting to coaches.',
        completionCriteria: 'Advance opportunities through Contacted → Proposal Sent → Negotiation, project deal value from discovery, and prepare a package for Director review.',
        demonstrateTask: 'Advance opportunities through the correct stages (Contacted → Proposal Sent → Negotiation), project deal value from discovery, and prepare a package.',
        philosophyPrinciple: 1, // We sell trust before apparel
        learnContent: [
            {
    ...
    },
    {
        code: 'ACAD-105',
        name: 'Order Handoff',
        description: 'The Closed Won standard, what Operations needs (roster, sizing, artwork, payment), and the Director QA question.',
        completionCriteria: 'Reach Closed Won correctly, complete all required information, and successfully hand off to Operations.',
        demonstrateTask: 'Reach Closed Won, complete all required information (roster, sizing, artwork, payment), and hand off to Operations.',
    ...
    },
    {
        ...
        description: 'The 4-lane program system, 7 quality standards, 8 collections — know what TUF builds and why.',
        completionCriteria: 'Pass the product knowledge quiz (80%) and deliver a 5-minute product walkthrough to your Director.',
        demonstrateTask: 'Deliver a 5-minute product walkthrough to your Director covering one collection, three quality standards with physical samples, and the 4-lane system.',
        philosophyPrinciple: 1, // We sell trust before apparel
        learnContent: [
            {
...
export function getQuizResult(code) {
...
export function getMissionStatement(userId) {
    try {
...
export function saveMissionStatement(userId, text) {
    try {
        const raw = localStorage.getItem(MISSION_KEY);
...
export function getCoachReview(code) {
    const reviews = getCoachReviews();
    return reviews[code] ?? null;
}
export function saveCoachReview(code, review) {
    try {
        const reviews = getCoachReviews();
        reviews[code] = review;
...
export function getAcknowledgments(userId) {
    try {
...
export function isModuleAcknowledged(userId, code) {
    return getAcknowledgments(userId).has(code);
}
export function acknowledgeModule(userId, code) {
    try {
        const raw = localStorage.getItem(ACKNOWLEDGMENTS_KEY);
        const data = raw ? JSON.parse(raw) : {};
 ...
 */
export function detectAcad101(userId) {
 ...
 */
export async function detectAcad102() {
 ...
 */
export async function detectAcad103() {
    const user = getStoredUser();
    ...
    const oppsWithNeeds = repOpps.filter((o) => (o.nextAction && o.nextAction.trim().length > 0));
    return {
 ...
 */
export async function detectAcad104() {
    const user = getStoredUser();
    const opportunities = await listOpportunities({});
    const repOpps = user
        ? opportunities.filter((o) => o.assignedRep === user.name)
    ...
    const proposalOpps = repOpps.filter((o) => proposalStages.has(o.stage) && (o.value ?? o.estimatedValue ?? 0) > 0);
    return {
 ...
 */
export async function detectAcad105() {
    const user = getStoredUser();
    const opportunities = await listOpportunities({});
    const repOpps = user
        ? opportunities.filter((o) => o.assignedRep === user.name)
...
export function detectAcad106() {
    const user = getStoredUser();
    if (!user)
        return { completed: false, currentValue: 0 };
 ...
 */
function computeModulePhase(code, exerciseCompleted, currentValue, userId, isAllCertified) {
    ...
    if (exerciseCompleted && quizPassed)
 ...
 */
export async function detectAllModules() {
    const user = getStoredUser();
 ...
 */
export function isLevel1Complete(progress) {
    return progress.every((p) => p.phase === 'acknowledged' || p.phase === 'certified');
}
/**
 * Returns certification progress percentage based on acknowledged modules.
 */
export function certificationProgress(progress) {
    const done = progress.filter((p) => p.phase === 'acknowledged' ||
        p.phase === 'certified' ||
        p.phase === 'coach_review').length;
 ...
 */
export function verifiedModuleCount(progress) {
    return progress.filter((p) => p.phase === 'awaiting_coach' ||
        p.phase === 'coach_review' ||
        p.phase === 'acknowledged' ||
        p.phase === 'certified').length;
}
...
export function getCertificationRecord(userId) {
    try {
 ...
 */
export function saveCertificationRecord(record) {
    try {
        const raw = localStorage.getItem(CERTIFICATION_KEY);
        const records = raw
            ? JSON.parse(raw)
...
export function getAllCertificationRecords() {
    try {
        const raw = localStorage.getItem(CERTIFICATION_KEY);
        if (!raw)
            return [];
...
export async function submitForApproval(userId, userName) {
    const progress = await detectAllModules();
    const quizResults = getQuizResults();
    const moduleDetails = LEVEL_1_MODULES.map((mod) => {
        ...
        return {
            ...
            exerciseVerified: prog?.phase === 'awaiting_coach' ||
...
export function clearSubmission(userId) {
    try {
        const raw = localStorage.getItem(SUBMISSION_KEY);
        if (raw) {
            const submissions = JSON.parse(raw);
 ...
 */
export async function directorApproveRep(repUserId, repUserName, directorName) {
    ...
    if (!allQuizzesPassed || !allExercisesVerified)
        return null;
    const progress = await detectAllModules();
    const record = {
        userId: repUserId,
 ...
 */
export async function directorCertifyRep(repUserId, repUserName, directorName) {
    return directorApproveRep(repUserId, repUserName, directorName);
}
export function updateUserCertificationStatus(userId, isCertified) {
    try {
        ...
        if (usersRaw) {
            ...
            const updated = users.map((u) => u.id === userId ? { ...u, isCertified } : u);
 ...
 */
async function callCertifyApi(userId) {
 ...
 */
export function resetAllCertifications() {
    ...
    try {
        ...
        if (usersRaw) {
            const users = JSON.parse(usersRaw);
            const updated = users.map((u) => u.role === 'REP'
                ? { ...u, isCertified: false }
                : u);
            localStorage.setItem('tuf_ops_users_v7', JSON.stringify(updated));
        }
