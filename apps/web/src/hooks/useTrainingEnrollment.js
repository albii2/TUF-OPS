...
export const ACADEMY_PHASE_LABELS = {
...
export const ACADEMY_CERTIFICATION_LABELS = {
...
const DEFAULT_TRAINING_MODULES = rawModules
...
function buildDefaultEnrollment(userId, role = getCurrentTrainingRole()) {
...
export function useTrainingEnrollment(userId) {
            ...
            try {
                ...
                if (cached) {
                    try {
                        ...
                        const hasOldModules = parsed.modules?.some((m) => m.title === 'Welcome to TUF Sports Apparel' || !m.title.startsWith('Playbook'));
                        if (hasOldModules) {
...
export function useTrainingModule(moduleId, enrollmentId) {
    const [moduleData, setModuleData] = useState(null);
    const [loading, setLoading] = useState(false);
    const getUserIdFromLocalStorage = () => {
        const raw = localStorage.getItem('tuf_ops_user_v3');
        if (!raw)
            return 'u-rep-jason-mulder'; // fallback
        try {
            const parsed = JSON.parse(raw);
    ...
    const updateLocalProgress = (modId, status, timeSpentSeconds) => {
    ...
    const completeModule = async (timeSpentSeconds) => {
        try {
            setLoading(true);
...
## Practical Checkpoint
*/
//# sourceMappingURL=useTrainingEnrollment.js.map