...
function buildRow(rep, opportunities, orders, organizations) {
    ...
    const latestActivity = Math.max(...opportunities.filter((o) => o.assignedRep === rep && o.lastActivity).map((o) => new Date(o.lastActivity).getTime()), 0);
...
export function EarningsPage() {
    ...
    const laneRevenue = ['UNIFORM', 'TRAVEL_GEAR', 'TEAM_STORE', 'LETTERMAN'].map((lane) => ({ lane, value: opportunities.filter((o) => o.stage === 'CLOSED_WON' && o.lanes.includes(lane)).reduce((sum, o) => sum + o.value, 0) }));
        ...
        <Card title="Rep Pace And Activity">
          <div className="space-y-2">
            {focusRows.map((row) => {
                ...
                return (<div key={row.rep} className="grid gap-3 rounded-lg border border-slate-800 bg-slate-950/70 p-3 lg:grid-cols-[1fr_1fr_1fr_1fr_1fr]">
                  <div>
    ...
    return (<div className="space-y-3">
      ...
      <Card title={user?.role === 'REP' ? 'My 4-Order Pace' : 'Team 4-Order Pace'}>
        <div className="space-y-2">
          {visibleRows.map((row) => {
            const remaining = Math.max(MONTHLY_ORDER_GOAL - row.wonCount, 0);
            return (<div key={row.rep} className="grid gap-3 rounded-lg border border-slate-800 bg-slate-950/70 p-3 lg:grid-cols-[1fr_1fr_1fr_1fr]">
                <div>
                  <p className="font-semibold text-white">{row.rep}</p>
                  <p className="text-sm text-slate-400">{remaining ? `${remaining} order${remaining === 1 ? '' : 's'} from target` : 'Target reached'}</p>
                </div>
