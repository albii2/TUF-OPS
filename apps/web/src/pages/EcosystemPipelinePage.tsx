...
import { Button, Card, DataTable, Input, Select, SmallKpi, type Column } from '../components/primitives';
import { useEcosystemReferrals } from '../hooks/useEcosystemReferrals';
import {
  getEcosystemReferralSummary,
  getReferralRepEffectiveness,
  getReferralSourceEffectiveness,
  warmIntroductionStatuses,
  type EcosystemReferral,
  type WarmIntroductionStatus,
} from '../services/ecosystemReferralsService';
import { formatCurrency } from '../utils/format';

const statusTone: Record<WarmIntroductionStatus, string> = {
...
function StatusBadge({ status }: { status: WarmIntroductionStatus }) {
...
export function EcosystemPipelinePage() {
  const [status, setStatus] = useState<'ALL' | WarmIntroductionStatus>('ALL');
  ...
  const columns: Column<EcosystemReferral>[] = [
      ...
      <Card title="Ecosystem Pipeline">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Select value={status} onChange={(e) => setStatus(e.target.value as 'ALL' | WarmIntroductionStatus)}>
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <Card title="Source Effectiveness by Organization">
          <div className="space-y-2">
            {sourceEffectiveness.slice(0, 8).map((row) => (
        ...
        <Card title="Referral Effectiveness by Rep">
          <div className="space-y-2">
            {repEffectiveness.slice(0, 8).map((row) => (
