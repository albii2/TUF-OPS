...
import { loadQualifications, getQualificationProgress, checkAndAwardQualifications, } from '../lib/achievements';
    ...
    useEffect(() => {
        async function computeStats() {
            try {
                ...
                const activeOpps = opps.filter((o) => o.stage !== 'CLOSED_WON' && o.stage !== 'CLOSED_LOST').length;
                ...
                const oppsThisMonth = opps.filter((o) => o.stage === 'CLOSED_WON' && o.closedAt && o.closedAt >= monthStart);
                const oppsThisQuarter = opps.filter((o) => o.stage === 'CLOSED_WON' && o.closedAt && o.closedAt >= quarterStart);
                ...
                try {
                    ...
                    if (academyRaw) {
                        ...
                        if (academy?.quizResults) {
                            quizModulesPassed = Object.values(academy.quizResults).filter((r) => r?.passed === true).length;
                        }
                    }
                }
                catch { /* ignore */ }
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="rounded-lg border border-slate-700/50 bg-slate-800/30 p-3">
                <div className="flex items-center justify-between">
                  ...
                  {user?.isCertified ? (<span className="rounded-full border border-emerald-500/50 bg-emerald-500/10 px-2 py-0.5 text-[10px] font-semibold text-emerald-300">
                      CERTIFIED
                    </span>) : (<span className="rounded-full border border-amber-500/50 bg-amber-500/10 px-2 py-0.5 text-[10px] font-semibold text-amber-300">
                      {stats.quizModulesPassed > 0 ? `${Math.round((stats.quizModulesPassed / 5) * 100)}%` : 'START'}
                    </span>)}
                </div>
                {!user?.isCertified && (<div className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-700">
                    <div className="h-full rounded-full bg-cyan-500 transition-all" style={{ width: `${Math.min(100, (stats.quizModulesPassed / 5) * 100)}%` }}/>
                  </div>)}
              </div>
        ...
        <div className="space-y-4">
          <Card title="Theme">
            <div className="space-y-3 text-sm">
              <div>
                ...
                <Select value={prefs.accent} onChange={(e) => setPrefs((p) => ({ ...p, accent: e.target.value }))}>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={prefs.compactMode} onChange={(e) => setPrefs((p) => ({ ...p, compactMode: e.target.checked }))} className="h-4 w-4 rounded border-slate-600 bg-slate-800 accent-cyan-500"/>
                <span className="text-slate-300">Compact Layout</span>
              </label>

              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={prefs.notifications} onChange={(e) => setPrefs((p) => ({ ...p, notifications: e.target.checked }))} className="h-4 w-4 rounded border-slate-600 bg-slate-800 accent-cyan-500"/>
              ...
              <div>
                ...
                <Select value={prefs.defaultView} onChange={(e) => setPrefs((p) => ({ ...p, defaultView: e.target.value }))}>
          ...
          <Card title="Favorite Sports (Ticker)">
            ...
            <div className="grid grid-cols-2 gap-2">
              {ALL_LEAGUES.map((league) => (<label key={league.value} className={`flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-xs transition ${prefs.favoriteLeagues.includes(league.value)
                ? 'border-cyan-500/50 bg-cyan-500/10 text-cyan-200'
                : 'border-slate-700/50 bg-slate-800/30 text-slate-400 hover:border-slate-600'}`}>
                  <input type="checkbox" checked={prefs.favoriteLeagues.includes(league.value)} onChange={() => toggleLeague(league.value)} className="sr-only"/>
          ...
          <Card title="Notification Preferences">
            <div className="space-y-2 text-sm">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" defaultChecked className="h-4 w-4 rounded border-slate-600 bg-slate-800 accent-cyan-500"/>
          ...
          {user?.role === 'ADMIN' || user?.role === 'REGIONAL_DIRECTOR' ? (<Card title="Workspace">
              <div className="space-y-2 text-sm">
