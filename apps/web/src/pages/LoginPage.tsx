...
import { authenticateWithPin } from '../services/usersService';
...
export function LoginPage({ setUser }: { setUser: (u: AppUser | null) => void }) {
  ...
  const onSubmit = async (e: FormEvent) => {
    e.preventDefault();
    const matchedUser = await authenticateWithPin(accessPin);
    if (!matchedUser) {
      setError('Invalid 4-digit PIN');
      return;
    }
    const signedInUser = updateStoredUser(matchedUser);
    setUser(signedInUser);
    navigate(signedInUser.mustChangeCredential ? '/change-credential' : '/dashboard');
  };
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-tuf-texture p-4">
      <form onSubmit={onSubmit} className="w-full max-w-sm rounded-xl border border-slate-800 bg-slate-950/76 p-6 text-center shadow-[0_26px_70px_rgba(3,9,24,0.8)] backdrop-blur">
        <TufLogo />
        <p className="mt-4 text-xs uppercase tracking-[0.2em] text-slate-400">4-Digit Operator PIN</p>
        <input
          inputMode="numeric"
          pattern="[0-9]*"
          maxLength={4}
          ...
          className="mt-6 w-full rounded-xl border border-slate-700 bg-slate-900/80 px-4 py-3 text-center text-3xl tracking-[0.4em] text-cyan-200 focus:border-cyan-400 focus:outline-none"
          placeholder="0000"
        />
