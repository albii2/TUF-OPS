import { FormEvent, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { loginWithCredential } from '../auth';
import { TufLogo } from '../components/ui';
import type { AppUser } from '../types';
import TufMarkSvg from '../assets/tuf-mark.svg';

export function LoginPage({ setUser }: { setUser: (u: AppUser | null) => void }) {
  const [login, setLogin] = useState('owner@tuf.local');
  const [credential, setCredential] = useState('');
  const [error, setError] = useState('');
  const navigate = useNavigate();

  const onSubmit = async (e: FormEvent) => {
    e.preventDefault();
    const user = await loginWithCredential(login, credential);
    if (!user) {
      setError('Invalid credential');
      return;
    }
    setUser(user);
    navigate(user.mustChangeCredential ? '/change-credential' : '/dashboard');
  };

  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-tuf-texture p-4">
      <form onSubmit={onSubmit} className="w-full max-w-sm rounded-xl border border-slate-800 bg-slate-950/76 p-6 text-center shadow-[0_26px_70px_rgba(3,9,24,0.8)] backdrop-blur">
        <TufLogo />
        <p className="mt-4 text-xs uppercase tracking-[0.2em] text-slate-400">Secure Operator Access</p>
        <input value={login} onChange={(e) => setLogin(e.target.value)} className="mt-6 w-full rounded-xl border border-slate-700 bg-slate-900/80 px-4 py-3 text-sm text-cyan-100 focus:border-cyan-400 focus:outline-none" placeholder="Email or name" />
        <input
          inputMode="numeric"
          pattern="[0-9]*"
          value={credential}
          onChange={(e) => setCredential(e.target.value.replace(/\D/g, ''))}
          className="mt-3 w-full rounded-xl border border-slate-700 bg-slate-900/80 px-4 py-3 text-center text-3xl tracking-[0.4em] text-cyan-200 focus:border-cyan-400 focus:outline-none"
          placeholder="Credential"
        />
        <button className="mt-6 w-full rounded-xl border border-cyan-400/50 bg-gradient-to-r from-cyan-500/20 to-blue-500/15 py-3 text-sm font-semibold uppercase tracking-[0.18em] text-cyan-100">
          Enter Command Center
        </button>
        {error ? <p className="mt-3 text-xs text-rose-300">{error}</p> : <p className="mt-3 text-xs text-slate-500">Use your individual PIN/password credential.</p>}
      </form>
      <img src={TufMarkSvg} alt="TUF mark" className="pointer-events-none mt-6 h-10 w-10 object-contain opacity-90" />
    </div>
  );
}
