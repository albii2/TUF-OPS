...
export default function LeadershipCommsPage() {
  ...
  const fetchComms = async () => {
    try {
      ...
      const res = await apiClient(`/comms${params}`) as { comms: Comm[] };
      setComms(res.comms || []);
  ...
  const handleUpdate = async (id: number, updates: Partial<Comm>) => {
    try {
    ...
    <div className="p-4 max-w-6xl mx-auto space-y-4">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setShowForm(!showForm)}
          className="px-3 py-1.5 bg-cyan-600 text-white rounded text-sm font-semibold hover:bg-cyan-500">
          + New Communication
        </button>
      </div>
      ...
      <div className="flex gap-1">
        {['all', 'draft', 'scheduled', 'sent', 'cancelled'].map(s => (
          <button key={s} onClick={() => setFilter(s)}
            className={`px-2 py-1 text-xs rounded ${filter === s ? 'bg-cyan-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}`}>
      ...
      {showForm && (
        <div className="bg-slate-900 border border-slate-700 rounded-lg p-4 space-y-3">
          ...
          <div className="flex gap-1 flex-wrap">
            ...
            {COMM_TYPES.map(t => (
              <button key={t.key} onClick={() => handleTemplate(t.key)}
                className={`text-[10px] px-1.5 py-0.5 rounded ${commType === t.key ? 'bg-cyan-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}`}>
                {t.label}
              </button>
            ))}
          </div>
          ...
          {/* Body */}
          <textarea placeholder="Communication body..." value={form.body}
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="flex items-center gap-2">
              ...
              <button onClick={scheduleForMorning}
                className="px-2 py-1 bg-purple-600 text-white rounded text-[10px] whitespace-nowrap">
                6:00 AM
              </button>
            </div>
            <input type="text" placeholder="Notes (internal only)" value={form.notes}
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setShowForm(false); setCommType(''); }}
              className="px-4 py-1.5 bg-slate-700 text-slate-300 rounded text-sm">Cancel</button>
          </div>
        </div>
      )}
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-2 mt-1 text-xs text-slate-500">
                      ...
                      {c.scheduled_for && (
                        <span className="text-purple-300">⚡ {new Date(c.scheduled_for).toLocaleString()}</span>
                      )}
                  ...
                  <div className="flex gap-1 shrink-0 ml-2">
                    {c.status === 'draft' && (
                      <button onClick={() => handleUpdate(c.id, { status: 'scheduled' })}
                        className="text-[10px] px-1.5 py-0.5 bg-purple-600 text-white rounded">Schedule</button>
                    )}
                    {c.status === 'scheduled' && (
                      <>
                        <button onClick={() => handleUpdate(c.id, { status: 'sent', sent_at: new Date().toISOString() })}
                          className="text-[10px] px-1.5 py-0.5 bg-emerald-600 text-white rounded">Mark Sent</button>
                        <button onClick={() => handleUpdate(c.id, { status: 'draft', scheduled_for: undefined })}
                          className="text-[10px] px-1.5 py-0.5 bg-slate-600 text-white rounded">Unschedule</button>
                      </>
                    )}
                    {(c.status === 'draft' || c.status === 'scheduled') && (
                      <button onClick={() => handleUpdate(c.id, { status: 'cancelled' })}
                        className="text-[10px] px-1.5 py-0.5 bg-red-600 text-white rounded">Cancel</button>
                    )}
                  </div>
                </div>
              </div>
