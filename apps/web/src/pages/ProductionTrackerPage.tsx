...
export function ProductionTrackerPage() {
  ...
  const ordersByStage = useMemo(() => {
    ...
    PRODUCTION_STAGES.forEach((stage) => {
      grouped[stage.key] = myOrders.filter(
        (order: Order) => order.productionStatus === stage.key
      );
    });
    return grouped;
  }, [myOrders]);
      ...
      <Card title="Production Tracker" className="min-w-0">
        ...
        {myOrders.length === 0 ? (
          <EmptyState
            ...
            description="Orders will appear here once deals are handed off to production."
          />
        ) : (
              ...
              return (
                <div key={stage.key}>
                  <button
                    ...
                    className="w-full rounded-lg border border-slate-800 bg-slate-950/70 p-4 text-left transition hover:border-cyan-500/50"
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span
                          className={`inline-flex items-center justify-center w-8 h-8 rounded-full text-xs font-bold border ${stage.color}`}
                        >
                          {orders.length}
                        </span>
                    ...
                    <div className="mt-2 h-1.5 bg-slate-800 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all ${
                          orders.length > 0 ? 'bg-cyan-400' : 'bg-slate-700'
                        }`}
                        style={{
                          width: `${Math.min(
                            (orders.length / Math.max(myOrders.length, 1)) * 100,
                            100
                          )}%`,
                        }}
                      />
                  ...
                  {isExpanded && orders.length > 0 && (
                    <div className="mt-1 space-y-1 pl-4">
                      {orders.map((order: Order) => (
                        <div
                          key={order.id}
                          className="rounded-lg border border-slate-800 bg-slate-950/50 p-3 cursor-pointer hover:border-cyan-500/50 transition"
