...
import { neededItemOptions } from '../services/creativeRequestsService';
...
const stageFlow = ['LEAD_ENGAGED', 'DISCOVERY', 'MOCKUP_STAGE', 'INVOICE_SENT', 'CLOSED_WON'];
...
function getActivityTypeInfo(message) {
    const isStageChange = message.startsWith('Stage advanced') ||
...
export function OpportunityDetailPage() {
    ...
    const [form, setForm] = useState({ requestType: 'MOCKUP', designTeam: 'APPAREL_MOCKUP', priority: 'NORMAL', title: '', sport: opp?.sport ?? '', season: opp?.season ?? '', neededItems: [], designNotes: '', inspirationNotes: '', dueDate: '', assetLinks: '', internalNotes: '' });
    ...
    if (!activeOpp)
        return <EmptyState title="Opportunity not found" description="Select another opportunity from the pipeline table."/>;
    const currentStageIndex = stageFlow.indexOf(activeOpp.stage);
    ...
    const setStage = async (stage, message) => {
        try {
            let updated;
    ...
    const logCrmActivity = async (message) => {
    ...
    const removeOpportunity = async () => {
        if (!window.confirm(`Remove opportunity "${activeOpp.title}" from the pipeline?`))
            return;
    ...
    return (<div className="space-y-3">
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-400">
              ...
              <span className="inline-flex flex-wrap gap-1">
                {activeOpp.lanes.map((lane) => (<span key={lane} className="inline-flex items-center gap-1 rounded-full border border-[#23557a] bg-[#0e2131] px-2 py-0.5 text-[10px] font-semibold text-[#cdeaff]">
                    {getLaneLabel(lane)}
                    <button className="ml-0.5 text-slate-500 hover:text-rose-400 leading-none" onClick={async () => {
                const result = await removeOpportunityLane(activeOpp.id, lane);
                if (result)
                    setLocalOpp(result);
            }} title="Remove lane">×</button>
                  </span>))}
                {inlineLaneEditing ? (<select className="rounded border border-slate-700 bg-slate-900 px-1 py-0.5 text-[10px] text-slate-100" value="" onChange={async (e) => {
                ...
                setInlineLaneEditing(false);
                    ...
                    {REVENUE_LANES.filter((l) => !activeOpp.lanes.includes(l)).map((lane) => (<option key={lane} value={lane}>{getLaneLabel(lane)}</option>))}
                  </select>) : (<button className="ml-1 text-slate-500 hover:text-cyan-300 text-[10px] underline" onClick={() => setInlineLaneEditing(true)}>+ Add Lane</button>)}
              </span>
            </div>
      ...
      <Card title="Close Path Timeline">
        <div className="flex flex-wrap gap-2">
          {opportunityStages.map((stage, idx) => {
            ...
            return (<div key={stage} className={`rounded-md border px-2 py-1 ${isCurrent ? 'border-cyan-400 bg-cyan-500/15' : done ? 'border-emerald-500/40 bg-emerald-500/10' : 'border-slate-800 bg-slate-950/70'}`}>
                <span className="text-[10px] text-slate-400">{idx + 1}</span> <StageBadge stage={stage}/>
              </div>);
        })}
          {nextStage ? (<div className="rounded-md border border-amber-400/50 bg-amber-500/10 px-2 py-1 text-xs font-semibold text-amber-200">
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        ...
        <Card title="Next Action Console"><div className='space-y-2'>
          ...
          {nextStage && canAdvance ? (<Button className="w-full" onClick={() => {
            ...
            }}>{stageCtas[activeOpp.stage]}</Button>) : nextStage && !canAdvance ? (<p className="text-sm text-slate-300">{getAdvanceDeniedMessage(activeOpp)}</p>) : null}
      ...
      {showAdvanceDrawer && nextStage && canAdvance ? (<div className="fixed inset-0 z-40 bg-black/60">
          <div className="absolute right-0 top-0 h-full w-full max-w-xl overflow-y-auto border-l border-slate-700 bg-[#08111a] p-4">
            ...
            <div className="space-y-3">
              {requiredAdvanceFields.map((field) => (<label key={field.key} className="block text-xs text-slate-300">
                  <span className="mb-1 block font-semibold text-slate-200">{field.label}</span>
                  {field.key === 'sport' ? (<select value={advanceForm[field.key] ?? ''} onChange={(e) => setAdvanceForm((prev) => ({ ...prev, [field.key]: e.target.value }))} className="mt-1 w-full rounded border border-slate-700 bg-slate-900 px-2 py-2 text-sm text-slate-100">
                      <option value="">Select sport…</option>
                      {SPORT_OPTIONS.map((sport) => (<option key={sport} value={sport}>{sport}</option>))}
                    </select>) : field.key === 'lane' ? (<select value={advanceForm[field.key] ?? ''} onChange={(e) => setAdvanceForm((prev) => ({ ...prev, [field.key]: e.target.value }))} className="mt-1 w-full rounded border border-slate-700 bg-slate-900 px-2 py-2 text-sm text-slate-100">
                      <option value="">Select lane…</option>
                      {REVENUE_LANES.map((lane) => (<option key={lane} value={lane}>{getLaneLabel(lane)}</option>))}
                    </select>) : (<input type={field.type ?? 'text'} value={advanceForm[field.key] ?? ''} onChange={(e) => setAdvanceForm((prev) => ({ ...prev, [field.key]: e.target.value }))} className="mt-1 w-full rounded border border-slate-700 bg-slate-900 px-2 py-2 text-sm text-slate-100"/>)}
                </label>))}
            </div>
            <div className="mt-4 flex gap-2">
              <Button onClick={async () => {
                ...
                if (newLane && !activeOpp.lanes.includes(newLane)) {
                    const laneResult = await addOpportunityLane(activeOpp.id, newLane);
                    if (laneResult)
                        setLocalOpp(laneResult);
                }
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        <Card title="Decision Timeline" className="lg:col-span-2">
          <div className="space-y-2 text-sm">
            ...
            return (<div key={entry.id} className="rounded-lg border border-slate-800 bg-slate-950/60 p-3 space-y-1 border-l-2" style={{ borderLeftColor: typeInfo.color }}>
      ...
      <Card title="Creative Requests">
        ...
        {showForm ? <div className="mb-3 rounded-lg border border-slate-800 bg-slate-950/60 p-3 space-y-2 text-sm">
          ...
          <div className="grid gap-2 md:grid-cols-3">
            <select className='rounded border border-slate-700 bg-slate-900 px-2 py-1' value={form.requestType} onChange={(e) => setForm({ ...form, requestType: e.target.value })}><option>MOCKUP</option><option>APPAREL_GRAPHIC</option><option>COLLECTION_DESIGN</option><option>SOCIAL_MEDIA_GRAPHIC</option><option>EVENT_LOGO</option><option>BRAND_DOCUMENT</option><option>SALES_FLYER</option><option>OTHER</option></select>
            <select className='rounded border border-slate-700 bg-slate-900 px-2 py-1' value={form.designTeam} onChange={(e) => setForm({ ...form, designTeam: e.target.value })}><option>APPAREL_MOCKUP</option><option>SOCIAL_BRAND</option></select>
            <select className='rounded border border-slate-700 bg-slate-900 px-2 py-1' value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value })}><option>LOW</option><option>NORMAL</option><option>HIGH</option><option>URGENT</option></select>
          </div>
          ...
          <Button className='w-full sm:w-auto' onClick={async () => { try {
            const request = await submitCreativeRequest({ opportunityId: activeOpp.id, organizationId: activeOpp.organizationId, assignedDesigner: '', requestType: form.requestType, designTeam: form.designTeam, priority: form.priority, title: form.title, sport: form.sport, season: form.season, neededItems: form.neededItems, designNotes: form.designNotes, inspirationNotes: form.inspirationNotes, dueDate: form.dueDate || undefined, assetLinks: form.assetLinks, internalNotes: form.internalNotes, trelloCardUrl: '' });
            setSuccess(request.trelloDispatchStatus === 'SENT' ? 'Creative request submitted and Trello card sent to the intern queue.' : request.trelloDispatchStatus === 'FAILED' ? `Creative request saved, but Trello dispatch failed: ${request.trelloDispatchError}` : 'Creative request saved. Configure Trello env vars to send cards to interns automatically.');
            setError('');
            setShowForm(false);
            setRefreshTick((x) => x + 1);
        }
        catch {
            setError('Unable to submit creative request. Please check required fields and try again.');
            setSuccess('');
      ...
      {/* ── Note Modal ── */}
      {showNoteModal ? (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="relative w-full max-w-lg rounded-2xl border border-slate-700 bg-[#070c13] p-6 shadow-2xl">
            ...
            <textarea value={noteText} onChange={(e) => setNoteText(e.target.value)} placeholder="Enter your note..." rows={5} className="w-full rounded-lg border border-slate-700 bg-slate-900/60 p-3 text-sm text-slate-200 placeholder-slate-600 focus:border-cyan-400/40 focus:outline-none resize-vertical"/>
            <div className="flex justify-end gap-2 mt-4">
              <Button className="border-slate-600 bg-slate-800/60 text-slate-200" onClick={() => setShowNoteModal(false)}>Cancel</Button>
              <Button onClick={() => { if (noteText.trim()) {
            logCrmActivity(noteText.trim());
      ...
      {/* ── Follow-Up Modal ── */}
      {showFollowUpModal ? (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="relative w-full max-w-lg rounded-2xl border border-slate-700 bg-[#070c13] p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
            ...
            <div className="space-y-3">
              <label className="block text-xs text-slate-300">
                ...
                <input type="date" value={followUpForm.date} onChange={(e) => setFollowUpForm((p) => ({ ...p, date: e.target.value }))} className="mt-1 w-full rounded border border-slate-700 bg-slate-900 px-2 py-2 text-sm text-slate-100"/>
              </label>
              <label className="block text-xs text-slate-300">
                <span className="mb-1 block font-semibold text-slate-200">Notes</span>
                <textarea value={followUpForm.notes} onChange={(e) => setFollowUpForm((p) => ({ ...p, notes: e.target.value }))} placeholder="Follow-up details..." rows={4} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900/60 p-3 text-sm text-slate-200 placeholder-slate-600 focus:border-cyan-400/40 focus:outline-none resize-vertical"/>
              </label>
            </div>
            <div className="flex justify-end gap-2 mt-4">
...
//# sourceMappingURL=OpportunityDetailPage.js.map