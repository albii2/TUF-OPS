...
export function ReportsPage() {
    ...
    const accountability = useMemo(() => reportsSummary.repPerformance.map((rep) => ({ ...rep, stale: staleOpps.filter((o) => o.assignedRep === rep.rep).length, nearClose: nearClose.filter((o) => o.assignedRep === rep.rep).length })), [reportsSummary, staleOpps, nearClose]);
    ...
    return (<div className="space-y-3">
      ...
      <Card title="Lane Performance">
        <div className="grid gap-2 md:grid-cols-2">
          {reportsSummary.lanePerformance.map((lane) => (<div key={lane.lane} className="rounded-lg border border-slate-800 bg-slate-950/60 p-3 text-sm">
      ...
      <Card title="Rep Performance">
        <div className="grid gap-2 md:grid-cols-2">
          {reportsSummary.repPerformance.map((rep) => (<div key={rep.rep} className="rounded-lg border border-slate-800 bg-slate-950/60 p-3 text-sm">
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          <div>
            ...
            <div className="space-y-2">
              {referralSourceEffectiveness.slice(0, 5).map((row) => (<div key={row.source} className="rounded-lg border border-slate-800 bg-slate-950/60 p-3 text-sm">
          ...
          <div>
            ...
            <div className="space-y-2">
              {referralRepEffectiveness.slice(0, 5).map((row) => (<div key={row.rep} className="rounded-lg border border-slate-800 bg-slate-950/60 p-3 text-sm">
