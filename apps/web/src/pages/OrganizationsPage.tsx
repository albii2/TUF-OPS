...
export function OrganizationsPage() {
  ...
  const filtered = useOrganizations({ search, status: status as 'ALL' | 'ACTIVE' | 'WATCH' | 'NEW', rep }).filter((o) => {
    ...
    if (coverageStatus === 'Untouched') return o.status === 'NEW';
    if (coverageStatus === 'Contacted') return o.status === 'WATCH';
    if (coverageStatus === 'Active') return o.status === 'ACTIVE';
    if (coverageStatus === 'Closed') return Object.values(o.laneStatuses).some((l) => l.status === 'WON');
    return true;
  });
  ...
  const columns: Column<(typeof filtered)[number]>[] = [
    ...
    { key: 'territory', header: 'Territory', cell: (r) => r.state === 'TX' ? 'TX Core' : 'MN Territory' },
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="mb-3 grid gap-2 lg:grid-cols-[1fr_180px_180px_180px_160px]">
        ...
        <Select value={coverageStatus} onChange={(e) => { setCoverageStatus(e.target.value); setPage(1); }}><option value='ALL'>Coverage</option><option>Untouched</option><option>Contacted</option><option>Active</option><option>Closed</option></Select>
