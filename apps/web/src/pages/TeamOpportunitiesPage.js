...
import { Button, Card, DataTable, EmptyState, LaneBadge, Select } from '../components/primitives';
...
export function TeamOpportunitiesPage() {
    ...
    const { data: lanes = [] } = useRevenueLanes();
    let rows = all.filter((o) => user?.role === 'ADMIN' || user?.role === 'DIRECTOR');
    if (rep !== 'ALL')
        rows = rows.filter((o) => o.assignedRep === rep);
    if (stage !== 'ALL')
        rows = rows.filter((o) => o.stage === stage);
    if (lane !== 'ALL')
        rows = rows.filter((o) => o.lanes.includes(lane));
    if (sport !== 'ALL')
        rows = rows.filter((o) => o.sport === sport);
    if (focus === 'NEAR_CLOSE')
        rows = getNearCloseOpportunities(rows);
    if (focus === 'STUCK')
        rows = getStuckOpportunities(rows);
    const columns = [
...
//# sourceMappingURL=TeamOpportunitiesPage.js.map