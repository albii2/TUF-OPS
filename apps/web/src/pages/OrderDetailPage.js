...
import { BLOCKER_FIELDS, ORDER_STAGE_FLOW, canAdvanceOrder, canSeeOrderValue, getAdvanceFields, getNextOrderStage, getOrderAdvanceWarning, getOrderDueDate, getOrderNextAction, getOrderOwner, getOrderRisk, getOrderStage, getOrderStageLabel, getOrderTitle, toProductionStatus, } from '../services/orderWorkflow';
import { getLaneLabel } from '../utils/naming';
function Chip({ label, value, tone = 'border-slate-700 bg-slate-900/60 text-slate-100' }) {
...
function FieldInput({ field, value, onChange }) {
    if (field.type === 'select') {
        return (<select value={value} onChange={(event) => onChange(event.target.value)} className="h-10 w-full rounded-lg border border-slate-800 bg-slate-900 px-3 text-sm text-white">
...
function buildPatchForStage(stage, form, existing) {
    ...
    if (stage === 'PAYMENT_CONFIRMED')
        return { ...base, paymentStatus: `Confirmed ${form.paymentDate || ''}`.trim() };
    if (stage === 'ARTWORK_FINALIZED')
        return { ...base, artworkStatus: `Approved ${form.approvalDate || ''}`.trim() };
    if (stage === 'VENDOR_READY')
        return { ...base, vendor: form.vendorSelected || existing.vendor, vendorStatus: 'Vendor ready' };
    if (stage === 'IN_PRODUCTION')
        return { ...base, vendorStatus: `Submitted ${form.vendorSubmissionDate || ''}`.trim() };
    if (stage === 'QUALITY_CHECK')
        return { ...base, vendorStatus: 'Production complete / QC', shippingStatus: form.issueNotes || 'Ready for shipment' };
    if (stage === 'SHIPPED_DELIVERED')
        return { ...base, shippingStatus: form.trackingInfo || 'Shipped / delivery pending' };
    if (stage === 'COMPLETED')
        return { ...base, completedDate: new Date().toISOString().slice(0, 10), shippingStatus: 'Delivery confirmed' };
    if (stage === 'BLOCKED_ON_HOLD')
        return { ...base, resolutionDueDate: form.resolutionDueDate, nextAction: `Resolve blocker: ${form.blockerReason}`, vendorNotes: form.notes || form.blockerReason };
    return base;
}
export function OrderDetailPage() {
    ...
    if (!activeOrder)
        return <EmptyState title="Order not found" description="Select a valid order from the execution queue."/>;
    ...
    return (<div className="space-y-3 min-w-0">
      ...
      <Card title="Stage Progress">
        <div className="grid gap-2 md:grid-cols-4 xl:grid-cols-8">
          {ORDER_STAGE_FLOW.map((flowStage, index) => {
            ...
            return (<div key={flowStage} className={`rounded-lg border p-2 ${isCurrent ? 'border-cyan-300 bg-cyan-400/15 text-cyan-50 ring-1 ring-cyan-300/40' : isComplete ? 'border-emerald-500/50 bg-emerald-500/10 text-emerald-100' : muted ? 'border-slate-800 bg-slate-950/40 text-slate-500' : 'border-slate-700 bg-slate-900/50 text-slate-300'}`}>
      ...
      {showAdvanceDrawer && drawerTargetStage ? (<div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm">
