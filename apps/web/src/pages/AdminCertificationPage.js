...
import { MODULE_ORDER, LEVEL_1_MODULES, CERTIFICATION_TITLE, getAllCertificationRecords, getAllSubmissions, directorApproveRep, saveCoachReview, getCoachReview, getMissionStatement, updateUserCertificationStatus, } from '../lib/academy';
...
function CoachReviewForm({ moduleCode, moduleName, repName, existingReview, onSave, onCancel, saving, }) {
    ...
    return (<div className="rounded-xl border border-purple-400/20 bg-[#0d1520] p-5 mt-4">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-black text-white">
          Coach Review: {moduleName} ({moduleCode}) — {repName}
        </h3>
        <button onClick={onCancel} className="rounded-lg border border-slate-600 px-3 py-1 text-xs text-slate-400 hover:bg-slate-800 transition-colors">
          Cancel
        </button>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <textarea value={strengths} onChange={(e) => setStrengths(e.target.value)} placeholder="E.g., The mission statement showed a clear understanding of TUF's purpose..." rows={3} className="w-full rounded-lg border border-slate-700 bg-slate-900/60 p-3 text-sm text-slate-200 placeholder-slate-600 focus:border-emerald-400/40 focus:outline-none resize-vertical"/>
        </div>
        ...
        <div>
          ...
          <textarea value={corrections} onChange={(e) => setCorrections(e.target.value)} placeholder="E.g., The proposal needs more detail on pricing options..." rows={3} className="w-full rounded-lg border border-slate-700 bg-slate-900/60 p-3 text-sm text-slate-200 placeholder-slate-600 focus:border-amber-400/40 focus:outline-none resize-vertical"/>
        </div>
        ...
        <div>
          ...
          <textarea value={coachingNotes} onChange={(e) => setCoachingNotes(e.target.value)} placeholder="E.g., Keep practicing discovery conversations. Remember to ask about ALL lanes..." rows={3} className="w-full rounded-lg border border-slate-700 bg-slate-900/60 p-3 text-sm text-slate-200 placeholder-slate-600 focus:border-cyan-400/40 focus:outline-none resize-vertical"/>
        </div>
      </div>

      <div className="mt-4 flex justify-end gap-3">
        <button onClick={onCancel} className="rounded-lg border border-slate-600 px-4 py-2 text-xs font-bold text-slate-400 hover:bg-slate-800 transition-colors">
          Cancel
        </button>
        <button onClick={handleSave} disabled={saving || !strengths.trim() || !corrections.trim() || !coachingNotes.trim()} className="rounded-lg border border-purple-400/40 bg-purple-400/10 px-4 py-2 text-xs font-bold text-purple-200 hover:bg-purple-400/20 disabled:opacity-40 transition-colors">
...
export default function AdminCertificationPage() {
    const user = getStoredUser();
    const isDirectorOrAdmin = user?.role === 'DIRECTOR' || user?.role === 'REGIONAL_DIRECTOR' || user?.role === 'ADMIN';
    const [records, setRecords] = useState([]);
    const [submissions, setSubmissions] = useState([]);
    const [allUsers, setAllUsers] = useState([]);
    const [approving, setApproving] = useState(null);
    const [selectedRep, setSelectedRep] = useState(null);
    const [pendingCoachReview, setPendingCoachReview] = useState(null);
    ...
    const handleApprove = async (repUser) => {
        if (!user)
            return;
    ...
    const forceCertifyRep = (repUser) => {
        // Update the current user session
        updateUserCertificationStatus(repUser.id, true);
        refreshData();
    };
    const handleSaveCoachReview = (review) => {
        if (!pendingCoachReview)
            return;
    ...
    const directors = allUsers.filter((u) => (u.role === 'DIRECTOR' || u.role === 'REGIONAL_DIRECTOR' || u.role === 'ADMIN') &&
        u.status === 'ACTIVE');
    const getRepRecord = (userId) => records.find((r) => r.userId === userId);
    const getRepSubmission = (userId) => submissions.find((s) => s.userId === userId);
    const getRepMission = (userId) => {
    ...
    const handleSendReminder = async (repId) => {
    ...
    const getStatusBadge = (rep) => {
        ...
        const verified = record?.moduleProgress.filter((m) => m.phase === 'awaiting_coach' ||
            m.phase === 'coach_review' ||
            m.phase === 'acknowledged' ||
            m.phase === 'certified').length ?? 0;
    ...
    const getModulePhaseClass = (phase) => {
        switch (phase) {
            case 'certified':
            case 'acknowledged':
    ...
    const getModulePhaseLabel = (phase) => {
        switch (phase) {
            case 'certified':
                return '✓';
            case 'acknowledged':
    ...
    const getModulePhaseTitle = (phase, code) => {
        switch (phase) {
            case 'certified':
    ...
    const getQuizScoreForModule = (repId, moduleCode) => {
    ...
    return (<div className="min-h-screen text-slate-100 p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <button onClick={refreshData} className="rounded-lg border border-slate-700 px-4 py-2 text-xs font-bold text-slate-300 hover:bg-slate-800 transition-colors">
            ↻ Refresh
          </button>
        </div>
        ...
        ].map(([key, label]) => (<button key={key} onClick={() => setFilter(key)} className={`rounded-md px-3 py-1.5 text-xs font-bold transition-colors ${filter === key
                ? 'bg-slate-700 text-white'
                : 'text-slate-400 hover:text-slate-300'}`}>
                {label}
              </button>))}
          </div>
          <button onClick={handleBulkReminder} disabled={sendingBulkReminder || uncertifiedCount === 0} className="rounded-lg border border-amber-400/30 bg-amber-400/5 px-4 py-2 text-xs font-bold text-amber-200 hover:bg-amber-400/10 disabled:opacity-40 transition-colors">
        ...
        <div className="rounded-xl border border-slate-800 bg-[#070c13]/80 overflow-hidden">
          <div className="overflow-x-auto">
            ...
            return (<tr key={rep.id} className="hover:bg-slate-900/30 transition-colors">
                        <td className="px-4 py-3">
                          <button onClick={() => setSelectedRep(selectedRep?.id === rep.id ? null : rep)} className="text-left hover:text-cyan-300 transition-colors">
                        ...
                        <td className="px-4 py-3 text-center">
                          <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${statusBadge.className}`}>
                            {statusBadge.label}
                          </span>
                          {statusBadge.detail && (<p className="text-[10px] text-slate-500 mt-1">
                              {statusBadge.detail}
                            </p>)}
                        </td>
                    ...
                    return (<td key={code} className="px-2 py-3 text-center">
                              <div className="flex flex-col items-center gap-1">
                                <span className={`inline-flex items-center justify-center w-8 h-8 rounded-full border text-xs font-bold ${getModulePhaseClass(phase)}`} title={getModulePhaseTitle(phase, code)}>
                                  {getModulePhaseLabel(phase)}
                                </span>
                                {quizScore !== null && quizScore > 0 && (<span className="text-[9px] text-slate-500 font-mono">
                                    {quizScore}%
                                  </span>)}
                                {coachReview && (<span className="text-[9px] text-purple-400" title="Has Coach Review">
                                    CR
                                  </span>)}
                              </div>
                            </td>);
                        ...
                        <td className="px-4 py-3 text-center">
                          {isAlreadyCertified ? (<span className="text-xs text-emerald-400 font-bold">
                              Certified ✓
                            </span>) : (<div className="flex items-center justify-center gap-1.5">
                              {hasSubmission && !isAlreadyCertified && (<button onClick={() => handleApprove(rep)} disabled={approving === rep.id} className="rounded-lg border border-purple-400/40 bg-purple-400/10 px-3 py-1.5 text-[10px] font-bold text-purple-200 hover:bg-purple-400/20 hover:border-purple-400/60 transition-colors disabled:opacity-50">
                                  {approving === rep.id ? '...' : 'Approve'}
                                </button>)}
                              {!isAlreadyCertified && isDirectorOrAdmin && (<button onClick={() => forceCertifyRep(rep)} title="Bypass quiz requirements and certify immediately." className="rounded-lg border border-amber-400/40 bg-amber-400/10 px-3 py-1.5 text-[10px] font-bold text-amber-200 hover:bg-amber-400/20 hover:border-amber-400/60 transition-colors">
                                  Force Certify
                                </button>)}
                              {!hasSubmission && !isDirectorOrAdmin && (<span className="text-[10px] text-slate-600">
        ...
        {selectedRep && (<div className="rounded-xl border border-cyan-400/15 bg-[#0d1520] p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
              ...
              <button onClick={() => setSelectedRep(null)} className="rounded-lg border border-slate-600 px-3 py-1 text-xs text-slate-400 hover:bg-slate-800 transition-colors">
                Close
              </button>
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
              {MODULE_ORDER.map((code) => {
                ...
                const coachReview = getCoachReview(code);
                return (<div key={code} className="rounded-lg border border-slate-800 bg-slate-900/40 p-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-bold text-cyan-400 uppercase">
                        {code}
                      </span>
                      <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full border text-xs font-bold ${getModulePhaseClass(phase)}`} title={getModulePhaseTitle(phase, code)}>
                        {getModulePhaseLabel(phase)}
                      </span>
                    </div>
                    ...
                    {/* Coach Review Status */}
                    {coachReview ? (<div className="mt-2 rounded-lg border border-purple-400/20 bg-purple-400/5 p-2.5">
                      ...
                      </div>) : phase === 'awaiting_coach' ? (<div className="mt-2">
                        ...
                        {pendingCoachReview &&
                            ...
                            pendingCoachReview.moduleCode === code ? (<CoachReviewForm moduleCode={code} moduleName={moduleDef?.name ?? code} repName={selectedRep.displayName} existingReview={null} onSave={handleSaveCoachReview} onCancel={() => setPendingCoachReview(null)} saving={savingReview}/>) : (<button onClick={() => setPendingCoachReview({
                      ...
                      </div>) : coachReview && phase === 'coach_review' ? (<div className="mt-2">
                        ...
                        {pendingCoachReview &&
                            pendingCoachReview.repId === selectedRep.id &&
                            pendingCoachReview.moduleCode === code ? (<CoachReviewForm moduleCode={code} moduleName={moduleDef?.name ?? code} repName={selectedRep.displayName} existingReview={coachReview} onSave={handleSaveCoachReview} onCancel={() => setPendingCoachReview(null)} saving={savingReview}/>) : (<button onClick={() => setPendingCoachReview({
                                repId: selectedRep.id,
                                repName: selectedRep.displayName,
                                moduleCode: code,
                                moduleName: moduleDef?.name ?? code,
                      ...
                      </div>) : phase === 'demonstrate' || phase === 'quiz_passed' ? (<div className="mt-2 rounded-lg border border-cyan-400/20 bg-cyan-400/5 p-2.5">
                        <p className="text-[10px] text-cyan-300">
                          Exercise in progress
                        </p>
                      </div>) : phase === 'acknowledged' || phase === 'certified' ? (<div className="mt-2 rounded-lg border border-emerald-400/20 bg-emerald-400/5 p-2.5">
        ...
        {submissions.length > 0 && (<div>
            ...
            <div className="space-y-4">
              {submissions
                ...
                return (<div key={sub.userId} className="rounded-xl border border-purple-400/15 bg-[#0d1520] p-5">
                      <div className="flex items-center justify-between mb-4">
                        <div>
                      ...
                      <table className="w-full text-sm">
                        ...
                        return (<tr key={detail.code} className="hover:bg-slate-900/20">
                                <td className="px-3 py-2.5">
                                  <span className="font-bold text-white">{detail.code}</span>
                                </td>
                                <td className="px-3 py-2.5 text-center">
                                  <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase ${detail.quizPassed
                                ...
                                <td className="px-3 py-2.5 text-center">
                                  {detail.quizPassed ? (<span className="text-emerald-400 font-bold">✓</span>) : (<span className="text-red-400 font-bold">✗</span>)}
                                </td>
                                <td className="px-3 py-2.5 text-center">
                                ...
                                <td className="px-3 py-2.5 text-center">
                                  {detail.exerciseVerified ? (<span className="text-emerald-400 font-bold">✓ Verified</span>) : (<span className="text-slate-500 text-xs">
                                      {detail.exerciseValue}/{detail.exerciseTarget}
                                    </span>)}
                                </td>
                                <td className="px-3 py-2.5 text-center">
                                  <div className="w-20 mx-auto h-1.5 bg-slate-800 rounded-full overflow-hidden">
                                    <div className={`h-full rounded-full ${detail.exerciseVerified
                                ? 'bg-emerald-400'
                                : 'bg-cyan-400'}`} style={{
                                width: `${Math.min((detail.exerciseValue / detail.exerciseTarget) * 100, 100)}%`,
                            }}/>
                                  </div>
                                </td>
                                <td className="px-3 py-2.5 text-center">
                                  {coachReview ? (<span className="text-xs text-purple-400 font-bold">✓</span>) : (<span className="text-xs text-slate-600">—</span>)}
                                </td>
                              </tr>);
                    })}
                        </tbody>
                      ...
                      {rep && (<div className="mt-4 flex justify-end">
                          <button onClick={() => handleApprove(rep)} disabled={approving === rep.id} className="rounded-lg border border-purple-400/40 bg-purple-400/10 px-4 py-2 text-sm font-bold text-purple-200 hover:bg-purple-400/20 disabled:opacity-40 transition-colors">
                            {approving === rep.id
                            ? 'Approving...'
        ...
        {directors.length > 0 && (<div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {directors.map((dir) => {
                ...
                return (<div key={dir.id} className="rounded-xl border border-cyan-400/15 bg-[#0d1520] p-4">
                    <div className="flex items-center justify-between">
                      <div>
...
//# sourceMappingURL=AdminCertificationPage.js.map