...
const pipelineStages = ['LEAD_ENGAGED', 'DISCOVERY', 'MOCKUP_STAGE', 'INVOICE_SENT', 'CLOSED_WON'];
function MetricTile({ value, label, tone, to }) {
    return (<Link to={to} className={`group rounded-lg border p-3 transition hover:-translate-y-0.5 hover:border-cyan-300/70 ${tone}`}>
...
function DealRow({ title, meta, value, to, nextAction, dueDate }) {
    return (<Link to={to} className="flex items-center justify-between gap-3 rounded-lg border border-slate-800 bg-slate-950/70 px-3 py-2 transition hover:border-cyan-400/70 hover:bg-slate-900">
...
function StagePipeline({ counts, values }) {
    return (<div className="grid gap-2 md:grid-cols-5">
      {pipelineStages.map((stage) => (<div key={stage} className="rounded-lg border border-slate-800 bg-slate-950/70 p-3">
...
function ActivityFeed({ title, activities }) {
    return (<GlassCard title={title}>
      <div className="space-y-2">
        {activities.length ? activities.map((activity) => (<div key={activity.id} className="rounded-lg border border-slate-800 bg-slate-950/70 p-3">
          ...
          </div>)) : <p className="text-sm text-slate-400">No activity logged yet.</p>}
      </div>
    </GlassCard>);
}
