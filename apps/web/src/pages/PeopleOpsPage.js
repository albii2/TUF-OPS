...
export default function PeopleOpsPage() {
    ...
    const handleAdvance = async (id, stage) => {
    ...
    return (<div className="p-4 max-w-6xl mx-auto space-y-4">
      ...
      <div className="grid grid-cols-7 gap-1">
        {ALL_STAGES.slice(0, 7).map(s => (<div key={s} className="bg-slate-900 border border-slate-800 rounded p-2 text-center">
      ...
      <div className="grid grid-cols-6 gap-1">
        {ALL_STAGES.slice(7).map(s => (<div key={s} className="bg-slate-900 border border-slate-800 rounded p-2 text-center">
            <p className="text-lg">{STAGE_EMOJI[s]}</p>
            <p className="text-xs text-slate-400 mt-1">{s.replace(/_/g, ' ')}</p>
            <p className="text-lg font-bold text-cyan-200">{stats[s] || 0}</p>
          </div>))}
      ...
      {showForm && (<div className="bg-slate-900 border border-slate-700 rounded-lg p-4 space-y-2 grid grid-cols-2 gap-2">
        ...
        {candidates.map(c => {
            ...
            return (<div key={c.id} className="bg-slate-900 border border-slate-800 rounded-lg p-3">
              ...
              <div className="flex gap-1 mt-2 flex-wrap">
                {ALL_STAGES.map(s => (<button key={s} onClick={() => handleAdvance(c.id, s)} className={`text-[10px] px-1.5 py-0.5 rounded ${c.current_stage === s ? 'bg-cyan-600 text-white' : 'bg-slate-800 text-slate-500 hover:text-white'}`}>
                    {STAGE_EMOJI[s]}
                  </button>))}
              </div>
            </div>);
...
//# sourceMappingURL=PeopleOpsPage.js.map