...
export default function DailyActivityCommand() {
  ...
  useEffect(() => {
    if (myEntry) {
      ...
      METRICS.forEach(({ key }) => { c[key] = String((myEntry as any)[key] || 0); });
      setCounts(c);
    }
  }, [myEntry]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    METRICS.forEach(({ key }) => {
      ...
      if (!isNaN(val)) input[key] = val;
    });
    await saveActivity(input as any);
  };
    ...
    <div className="max-w-5xl mx-auto p-6">
      ...
      {isAdmin && (
        <div className="mb-8 bg-gray-900 border border-gray-800 rounded-lg p-5">
          ...
          {today.length === 0 ? (
            <p className="text-gray-500 text-sm">No activity logged yet today. Reps — get your numbers in.</p>
          ) : (
                ...
                <thead>
                  <tr className="text-gray-500 border-b border-gray-800">
                    ...
                    {METRICS.map(({ key, label }) => (
                      <th key={key} className="text-center py-2 px-1 font-medium">{label.slice(0, 4)}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {today.map((entry) => (
                    <tr key={entry.id} className="border-b border-gray-800/50">
                      ...
                      {METRICS.map(({ key }) => (
                        <td key={key} className={`text-center py-2 ${key === 'conversations' || key === 'meetings' ? 'text-green-400 font-bold' : 'text-gray-300'}`}>
                          {(entry as any)[key] || 0}
                        </td>
                      ))}
                    </tr>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-lg p-5">
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-3 mb-4">
            {METRICS.map(({ key, label, emoji, highlight }) => (
              <div key={key}>
                ...
                <input
                  ...
                  className={`w-full bg-gray-800 border rounded px-2 py-1.5 text-center text-white text-sm
          ...
          <button
            type="submit"
            disabled={saving}
            className="bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50 text-white px-6 py-2 rounded text-sm font-semibold transition"
          >
