...
interface DashboardData {
  ...
  ops: any[];
  waiting: any[];
  dailyBrief: Record<string, number>;
}
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Section title="🎯 Recruiting Pipeline" count={(data.recruiting || []).length} color="border-purple-500">
              {(data.recruiting || []).map((r: any) => (
                <div key={r.name + r.stage} className="flex justify-between text-sm py-1 border-b border-slate-800">
            ...
            <Section title="📊 Near-Close Pipeline" count={(data.sales || []).length} color="border-emerald-500">
              {(data.sales || []).map((s: any) => (
                <div key={s.id} className="text-sm py-1 border-b border-slate-800">
                  <span className="text-white">{s.title || `Deal #${s.id}`}</span>
                  <span className="text-emerald-300 ml-2">{s.stage?.replace(/_/g, ' ')}</span>
                  {s.expected_value && (
                    <span className="text-slate-500 ml-2">${Number(s.expected_value).toLocaleString()}</span>
                  )}
                </div>
              ))}
            </Section>
            ...
            <Section title="⚙️ Active Operations" count={(data.ops || []).length} color="border-blue-500">
              {(data.ops || []).map((o: any) => (
                <div key={o.id} className="text-sm py-1 border-b border-slate-800">
            ...
            <Section title="⏳ Unassigned Items" count={(data.waiting || []).length} color="border-pink-500">
              {(data.waiting || []).map((w: any) => (
                <div key={w.id} className="text-sm py-1 border-b border-slate-800">
...
function DailyBriefCard({ brief }: { brief: Record<string, number> }) {
  const items = [
  ...
  return (
    <div className="bg-slate-900 border border-slate-700 rounded-lg p-4">
      ...
      <div className="grid grid-cols-4 gap-2">
        {items.map(i => (
          <div key={i.label} className="text-center bg-slate-800 rounded p-2">
