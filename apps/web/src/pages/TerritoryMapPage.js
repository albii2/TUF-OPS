...
export function TerritoryMapPage() {
    ...
    return (<div className="space-y-3">
      <Card title="TUF MINNESOTA TERRITORY MAP">
        ...
        <div className="grid gap-3 lg:grid-cols-[1.4fr_1fr]">
          ...
          <Card title={active ? `${active.name.toUpperCase()} SNAPSHOT` : 'ZONE SNAPSHOT'}>
            {active ? (<div className="space-y-1 text-sm text-[var(--text-primary)]">
      ...
      <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
        {zoneMetrics.map((zone) => (<button key={zone.id} onClick={() => setActiveZone(zone.id)} className={`rounded-lg border p-3 text-left ${activeZone === zone.id ? 'border-[#1FB6FF] glow-blue' : 'border-[var(--border)] panel-elevated'}`}>
...
//# sourceMappingURL=TerritoryMapPage.js.map