...
import { Card, DataTable, SmallKpi } from '../components/primitives';
...
export function TerritoryPage() {
    ...
    const repRows = listUsers()
        ...
        .filter((m) => {
        if (managedRepNames?.has(m.displayName))
            return true;
        const hasScopedTerritory = m.territory ? scopedZones.has(m.territory) : false;
    ...
    return (<div className="space-y-3">
      ...
      <Card title="Accounts Needing Action">
        <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
          {scopedStale.slice(0, 6).map((org) => (<Link key={org.id} to={`/organizations/${org.id}`} className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 transition hover:border-cyan-400">
...
//# sourceMappingURL=TerritoryPage.js.map