...
export function OrganizationNewPage() {
    ...
    const onSubmit = async (e) => {
        ...
        try {
            const created = await createOrganization({ name: normalizeAccountName(name), accountType, city, state, territory: territory, assignedRep: assignedRep || 'Unassigned Rep', assignedDirector: assignedDirector || 'Unassigned' });
...
export function OpportunityNewPage() {
    ...
    useEffect(() => {
        if (!organizationId && organizations[0]) {
            ...
            if (user?.role !== 'REP')
                setAssignedRep(organizations[0].assignedRep);
        }
    ...
    const onSubmit = async (e) => {
        ...
        if (!((assignedRep || resolvedOrg?.assignedRep || user?.name) ?? '').trim())
            missing.push('Assigned Rep');
        if (!programLevel.trim())
            missing.push('Program Level');
        if (!sport.trim())
            missing.push('Sport');
        if (!seasonCode.trim())
            missing.push('Season');
    ...
    return <Card title="New Opportunity"><form onSubmit={onSubmit} className="grid gap-2 md:grid-cols-2"><div><label className="text-xs text-slate-400">Organization <span className="text-rose-300">*</span></label><Input value={orgNameInput} onChange={(e) => setOrgNameInput(e.target.value)} onBlur={() => setOrgNameInput(normalizeAccountName(orgNameInput))} placeholder="Type or select school name..."/></div><Select value={organizationId} onChange={(e) => { const next = organizations.find((org) => org.id === e.target.value); setOrganizationId(e.target.value); if (next && user?.role !== 'REP')
        setAssignedRep(next.assignedRep); }}>{organizations.map((org) => <option key={org.id} value={org.id}>{org.name}</option>)}</Select><Input value={assignedRep} onChange={(e) => setAssignedRep(e.target.value)} placeholder="Assigned Rep" disabled={user?.role === 'REP'}/><Select value={programLevel} onChange={(e) => setProgramLevel(e.target.value)}>{levels.map((l) => <option key={l}>{l}</option>)}</Select><Select value={sport} onChange={(e) => setSport(e.target.value)}>{SPORT_OPTIONS.map((s) => <option key={s}>{s}</option>)}</Select><Input value={seasonCode} onChange={(e) => setSeasonCode(e.target.value.toUpperCase())} placeholder={`Season code (${SEASON_CODES.join('/')})`}/><Select value={lane} onChange={(e) => setLane(e.target.value)}>{REVENUE_LANES.map((l) => <option key={l}>{l}</option>)}</Select><Input inputMode="numeric" value={value} onChange={(e) => setValue(e.target.value.replace(/[^\d]/g, ''))} placeholder="Estimated Value"/><p className="md:col-span-2 text-sm text-cyan-200">Display Name Preview: {preview}</p><Button type="submit" className="md:col-span-2">Create Opportunity</Button>{message ? <p className="text-sm text-amber-200 md:col-span-2">{message}</p> : null}</form></Card>;
}
//# sourceMappingURL=NewPages.js.map