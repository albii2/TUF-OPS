...
import { LEVEL_1_MODULES, SALES_PHILOSOPHY, QUIZZES, QUIZ_PASS_THRESHOLD, MODULE_ORDER, CERTIFICATION_TITLE, detectAllModules, isLevel1Complete, verifiedModuleCount, getQuizResults, gradeQuiz, submitForApproval, saveCertificationRecord, getSubmission, getMissionStatement, saveMissionStatement, getCoachReview, acknowledgeModule, markPageVisited, } from '../lib/academy';
// ─── Quiz Modal Component ──────────────────────────────────────────────────
function QuizModal({ moduleCode, moduleName, questions, quizResult, onClose, onSubmit, submitting, }) {
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-slate-700 bg-[#070c13] p-6 shadow-2xl">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={onClose} className="rounded-lg border border-slate-600 px-3 py-1 text-xs text-slate-400 hover:bg-slate-800 transition-colors">
            ✕
          </button>
        </div>

        {/* Previous Result */}
        {quizResult && (<div className={`mb-4 rounded-lg border p-3 text-xs ${quizResult.passed
        ...
        {/* Feedback Banner */}
        {feedback && (<div className={`mb-4 rounded-lg border p-3 text-xs font-medium ${feedback.passed
        ...
        <div className="space-y-5 max-h-[50vh] overflow-y-auto">
          {questions.map((q, i) => (<div key={q.id} className="rounded-lg border border-slate-800 bg-slate-900/40 p-4">
              ...
              <div className="space-y-2 mt-3">
                ...
                return (<label key={oi} className={`flex items-center gap-3 rounded-lg border p-3 cursor-pointer transition-colors text-sm ${isSelected
                        ? 'border-cyan-400/40 bg-cyan-400/10 text-cyan-200'
                        : 'border-slate-700 bg-slate-800/40 text-slate-300 hover:border-slate-600'}`}>
                      <input type="radio" name={`quiz-${moduleCode}-q${i}`} checked={isSelected} onChange={() => {
        ...
        <div className="flex justify-end mt-6">
          <button onClick={handleSubmit} disabled={submitting || answers.some((a) => a === -1)} className="rounded-lg border border-cyan-400/40 bg-cyan-400/10 px-5 py-2 text-sm font-bold text-cyan-200 hover:bg-cyan-400/20 disabled:opacity-40 transition-colors">
...
function MissionStatementModal({ existingText, onClose, onSave, }) {
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-slate-700 bg-[#070c13] p-6 shadow-2xl">
        ...
        <textarea value={text} onChange={(e) => setText(e.target.value)} placeholder="Write your mission statement here..." rows={8} className="w-full rounded-lg border border-slate-700 bg-slate-900/60 p-4 text-sm text-slate-200 placeholder-slate-600 focus:border-cyan-400/40 focus:outline-none resize-vertical"/>
        ...
        <div className="flex justify-between items-center mt-4">
          ...
          <button onClick={() => onSave(text.trim())} disabled={!canSave} className="rounded-lg border border-cyan-400/40 bg-cyan-400/10 px-5 py-2 text-sm font-bold text-cyan-200 hover:bg-cyan-400/20 disabled:opacity-40 transition-colors">
...
// ─── Coach Review Modal ────────────────────────────────────────────────────
function CoachReviewModal({ moduleName, moduleCode, coachReview, onClose, onAcknowledge, acknowledging, }) {
    return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-purple-400/20 bg-[#070c13] p-6 shadow-2xl">
        ...
        <div className="rounded-lg border border-purple-400/30 bg-purple-400/5 p-4">
          ...
          <button onClick={onAcknowledge} disabled={acknowledging} className="rounded-lg border border-purple-400/40 bg-purple-400/10 px-5 py-2 text-sm font-bold text-purple-200 hover:bg-purple-400/20 disabled:opacity-40 transition-colors">
...
// ─── Learn Content Modal ───────────────────────────────────────────────────
function LearnContentModal({ module, onClose, }) {
    return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-cyan-400/20 bg-[#070c13] p-6 shadow-2xl">
        ...
        <div className="space-y-4">
          {module.learnContent.map((section, idx) => (<div key={idx} className="rounded-lg border border-slate-800 bg-slate-900/40 p-4">
...
export default function AcademyPage() {
    ...
    const [activeLearn, setActiveLearn] = useState(null);
    const [showMissionModal, setShowMissionModal] = useState(false);
    const [activeCoachReview, setActiveCoachReview] = useState(null);
    ...
    const completeCount = moduleProgress.filter((m) => m.phase === 'acknowledged' || m.phase === 'certified').length;
    ...
    const statusColor = isCertified
        ...
        : approvalSubmitted
            ...
            : isRep
                ...
                : 'border-cyan-400/40 bg-cyan-400/10 text-cyan-200';
    const handleQuizSubmit = useCallback((answers) => {
        if (!activeQuiz)
            return;
    ...
    const handleTakeQuiz = (code) => {
        setActiveQuiz(code);
    };
    const handleOpenLearn = (code) => {
        setActiveLearn(code);
    };
    const handleSaveMission = (text) => {
        saveMissionStatement(userId, text);
        setShowMissionModal(false);
        refreshProgress();
    };
    const handleAcknowledge = useCallback((code) => {
    ...
    const handleSubmitForApproval = useCallback(async () => {
        if (!isLevel1Complete(moduleProgress)) {
            setApprovalError('All 5 modules must go through Coach Review and you must acknowledge each one before submitting for certification.');
            return;
        }
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="rounded-2xl border border-slate-700 bg-[#0a0a0a] p-6 md:p-8">
          <div className="flex flex-col items-center text-center">
            <div className="mx-auto max-w-3xl">
              ...
              <div className="mt-4">
                <span className={`inline-flex items-center gap-2 rounded border px-4 py-1.5 text-xs font-bold uppercase tracking-wider ${isCertified
            ...
            : approvalSubmitted
              ...
              {isRep && (<div className="mt-5 max-w-md mx-auto rounded-lg border border-slate-700 bg-slate-900/40 p-4 text-left">
                ...
                const statusText = phase === 'certified' || phase === 'acknowledged' ? 'COMPLETE' :
                    phase === 'demonstrate' || phase === 'quiz_passed' || phase === 'awaiting_coach' || phase === 'coach_review' ? 'IN PROGRESS' :
                        'LOCKED';
                const statusColor = phase === 'certified' || phase === 'acknowledged' ? 'text-slate-200' :
                    phase !== 'locked' ? 'text-slate-400' :
                        'text-slate-600';
                return (<div key={code} className="flex items-center justify-between text-xs">
              ...
              {isRep && isCertified && (<div className="mt-5 max-w-md mx-auto grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            const exerciseProgressPercent = phase === 'demonstrate' ||
            ...
            const showExerciseBar = phase === 'demonstrate' ||
                phase === 'awaiting_coach' ||
                phase === 'coach_review';
            return (<div key={module.code} className={`rounded-xl border p-5 transition-all ${phaseColors[phase] || phaseColors['locked']}`}>
                  {/* Module Header */}
                  <div className="flex items-start justify-between mb-2">
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                      {module.code}
                    </span>
                    <span className={`inline-flex items-center gap-1 rounded border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${phase === 'certified' || phase === 'acknowledged'
                  ...
                  {(phase === 'learn') && module.learnContent && (<details className="mb-3 rounded-lg bg-slate-950/50 border border-slate-700/30 overflow-hidden group">
                      ...
                      <div className="px-3 pb-3 pt-1 text-xs text-slate-300 leading-relaxed border-t border-slate-700/20 space-y-3">
                        ...
                        ? module.learnContent.map((section, i) => (<div key={i}>
                                {section.title && (<h4 className="text-xs font-bold text-blue-200 mb-1">{section.title}</h4>)}
                  ...
                  {/* Exercise Status */}
                  {showExerciseBar && progress && (<div className="mb-3 rounded-lg bg-slate-950/50 border border-slate-800/60 p-2.5">
                  ...
                  {/* Learn Phase */}
                  {phase === 'learn' && (<div className="mb-3 space-y-2">
                      <button onClick={() => handleOpenLearn(module.code)} className="w-full rounded-lg border border-slate-600/40 bg-slate-800/40 px-3 py-2 text-xs font-bold text-slate-300 hover:bg-slate-700/40 transition-colors">
                        Study Learning Content
                      </button>
                      <button onClick={() => handleTakeQuiz(module.code)} className="w-full rounded-lg border border-slate-600/40 bg-slate-800/40 px-3 py-2 text-xs font-bold text-slate-300 hover:bg-slate-700/40 transition-colors">
                        Take Assessment
                      </button>
                    </div>)}

                  {/* Quiz Passed / Demonstrate */}
                  {(phase === 'quiz_passed' || phase === 'demonstrate') && (<div className="mb-3 space-y-2">
                      {quizResult && (<div className="rounded-lg bg-slate-950/50 border border-slate-800/40 p-2.5 flex items-center justify-between">
                          <span className={`inline-flex items-center gap-1 rounded border px-2 py-0.5 text-[10px] font-bold uppercase ${quizResult.passed
                            ? 'bg-slate-700/40 text-slate-200 border-slate-500/40'
                            : 'bg-slate-800/40 text-slate-400 border-slate-600/40'}`}>
                            {quizResult.passed ? 'PASSED' : 'NOT PASSED'}: {quizResult.score}%
                          </span>
                          <button onClick={() => handleTakeQuiz(module.code)} className="rounded-lg border border-slate-600/40 bg-slate-800/40 px-3 py-1 text-[10px] font-bold text-slate-300 hover:bg-slate-700/40 transition-colors">
                            Retake Assessment
                          </button>
                        </div>)}
                      ...
                      {module.code === 'ACAD-101' && (<button onClick={() => setShowMissionModal(true)} className="w-full rounded-lg border border-slate-600/40 bg-slate-800/40 px-3 py-2 text-xs font-bold text-slate-300 hover:bg-slate-700/40 transition-colors">
                          Write Mission Statement
                        </button>)}
                      {module.code === 'ACAD-102' && (<div className="space-y-1.5">
                          <Link to="/organizations" className="block w-full rounded-lg border border-slate-600/40 bg-slate-800/40 px-3 py-2 text-xs font-bold text-slate-300 hover:bg-slate-700/40 transition-colors text-center">
                            Practice: Add Organizations →
                          </Link>
                          <Link to="/opportunities" className="block w-full rounded-lg border border-slate-600/40 bg-slate-800/40 px-3 py-2 text-xs font-bold text-slate-300 hover:bg-slate-700/40 transition-colors text-center">
                            Practice: Log Activities →
                          </Link>
                        </div>)}
                      {module.code === 'ACAD-103' && (<Link to="/opportunities" className="block w-full rounded-lg border border-slate-600/40 bg-slate-800/40 px-3 py-2 text-xs font-bold text-slate-300 hover:bg-slate-700/40 transition-colors text-center">
                          Practice: Create Discovery Opportunities →
                        </Link>)}
                      {module.code === 'ACAD-104' && (<Link to="/opportunities" className="block w-full rounded-lg border border-slate-600/40 bg-slate-800/40 px-3 py-2 text-xs font-bold text-slate-300 hover:bg-slate-700/40 transition-colors text-center">
                          Practice: Build Proposals →
                        </Link>)}
                      {module.code === 'ACAD-105' && (<Link to="/opportunities" className="block w-full rounded-lg border border-slate-600/40 bg-slate-800/40 px-3 py-2 text-xs font-bold text-slate-300 hover:bg-slate-700/40 transition-colors text-center">
                          Practice: Close & Hand Off →
                        </Link>)}
                    </div>)}

                  {/* Awaiting Coach */}
                  {phase === 'awaiting_coach' && (<div className="mb-3 rounded-lg border border-slate-700/40 bg-slate-900/30 p-3">
                  ...
                  {/* Coach Review Ready */}
                  {phase === 'coach_review' && coachReview && (<div className="mb-3 space-y-2">
                      ...
                      <button onClick={() => setActiveCoachReview(module.code)} className="w-full rounded-lg border border-purple-400/40 bg-purple-400/10 px-3 py-2 text-xs font-bold text-purple-200 hover:bg-purple-400/20 transition-colors">
                        View Coach Review & Acknowledge
                      </button>
                    </div>)}

                  {/* Acknowledged */}
                  {phase === 'acknowledged' && (<div className="mb-3 rounded-lg border border-slate-600/40 bg-slate-900/30 p-3">
                  ...
                  {/* Certified */}
                  {(phase === 'certified') && (<div className="mb-3 rounded-lg border border-slate-600/40 bg-slate-900/30 p-3">
                  ...
                  {/* Locked */}
                  {phase === 'locked' && (<div className="mb-3 rounded-lg bg-slate-950/40 border border-slate-700/50 p-2.5">
        ...
        <div className="border-t border-slate-800 pt-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        ].map((resource) => (<a key={resource.file} href={`/training/${resource.file}`} target="_blank" rel="noopener noreferrer" className="rounded-xl border border-slate-700/50 bg-slate-900/30 p-5 hover:border-slate-500/50 hover:bg-slate-800/30 transition-all group">
            ...
            {/* Email Template */}
            <a href="/training/FIRST_CONTACT_EMAIL_TEMPLATE.md" target="_blank" rel="noopener noreferrer" className="rounded-xl border border-slate-700/50 bg-slate-900/30 p-5 hover:border-slate-500/50 hover:bg-slate-800/30 transition-all group">
              <div className="flex items-start gap-3">
                <span className="flex-shrink-0 w-9 h-9 rounded-lg bg-slate-800/50 border border-slate-600/30 flex items-center justify-center text-lg">
                  ✉️
                </span>
            ...
            {/* Product Cheat Sheet */}
            <a href="/training/product-cheat-sheets.md" target="_blank" rel="noopener noreferrer" className="rounded-xl border border-slate-700/50 bg-slate-900/30 p-5 hover:border-slate-500/50 hover:bg-slate-800/30 transition-all group">
              <div className="flex items-start gap-3">
                <span className="flex-shrink-0 w-9 h-9 rounded-lg bg-slate-800/50 border border-slate-600/30 flex items-center justify-center text-lg">
                  🏷️
                </span>
            ...
            {/* Territory Map */}
            <a href="/training/territorymap_tuf_2026.png" target="_blank" rel="noopener noreferrer" className="rounded-xl border border-slate-700/50 bg-slate-900/30 p-5 hover:border-slate-500/50 hover:bg-slate-800/30 transition-all group">
              <div className="flex items-start gap-3">
                <span className="flex-shrink-0 w-9 h-9 rounded-lg bg-slate-800/50 border border-slate-600/30 flex items-center justify-center text-lg">
                  🗺️
                </span>
        ...
        {/* ── Quiz Modal ── */}
        {activeQuiz && (<QuizModal moduleCode={activeQuiz} moduleName={moduleDefMap.get(activeQuiz)?.name ?? activeQuiz} questions={QUIZZES[activeQuiz]} quizResult={quizResults[activeQuiz] ?? null} onClose={() => setActiveQuiz(null)} onSubmit={handleQuizSubmit} submitting={submittingQuiz}/>)}

        {/* ── Learn Content Modal ── */}
        {activeLearn && (<LearnContentModal module={moduleDefMap.get(activeLearn)} onClose={() => setActiveLearn(null)}/>)}

        {/* ── Mission Statement Modal ── */}
        {showMissionModal && (<MissionStatementModal existingText={missionStatement} onClose={() => setShowMissionModal(false)} onSave={handleSaveMission}/>)}

        {/* ── Coach Review Modal ── */}
        {activeCoachReview && getCoachReview(activeCoachReview) && (<CoachReviewModal moduleName={moduleDefMap.get(activeCoachReview)?.name ?? activeCoachReview} moduleCode={activeCoachReview} coachReview={getCoachReview(activeCoachReview)} onClose={() => setActiveCoachReview(null)} onAcknowledge={() => handleAcknowledge(activeCoachReview)} acknowledging={acknowledging}/>)}

        {/* ── Submit for Certification Section ── */}
        {isRep && completeCount === 5 && !approvalSubmitted && !isCertified && (<div className="rounded-2xl border border-slate-700/50 bg-slate-900/30 p-6 mt-6">
            <div className="flex items-start gap-3">
              <div>
                ...
                {approvalError && (<div className="mt-3 rounded-lg border border-red-400/20 bg-red-500/5 p-3 text-xs text-red-200">
                    {approvalError}
                  </div>)}
                <button onClick={handleSubmitForApproval} disabled={submittingForApproval} className="mt-2 rounded-lg border border-purple-400/40 bg-purple-400/10 px-5 py-2 text-sm font-bold text-purple-200 hover:bg-purple-400/20 disabled:opacity-40 transition-colors">
        ...
        {/* ── Submitted State ── */}
        {approvalSubmitted && !isCertified && (<div className="rounded-2xl border border-slate-700/50 bg-slate-900/30 p-6 mb-6">
            <div className="flex items-start gap-3">
              <div>
        ...
        {/* ── Sales Philosophy ── */}
        <div>
          <button onClick={() => setShowPhilosophy(!showPhilosophy)} className="w-full flex items-center justify-between rounded-xl border border-slate-700/50 bg-slate-900/30 p-4 hover:border-slate-500/50 transition-colors">
          ...
          {showPhilosophy && (<div className="mt-3 grid gap-3 md:grid-cols-2">
              {SALES_PHILOSOPHY.map((principle) => (<div key={principle.number} className="rounded-xl border border-slate-700/50 bg-slate-900/30 p-4 hover:border-slate-600/50 transition-colors">
                  <div className="flex items-start gap-3">
        ...
        {/* ── Footer Status Boxes ── */}
        {isRep && !isCertified && !approvalSubmitted && completeCount < 5 && (<div className="rounded-2xl border border-slate-700/50 bg-slate-900/30 p-6">
            <div className="flex items-start gap-3">
              <div>
        ...
        {isCertified && (<div className="rounded-2xl border border-emerald-400/25 bg-emerald-500/5 p-6 backdrop-blur-md">
            <div className="flex items-start gap-3">
              <div>
                <h2 className="text-lg font-black text-white">
                  {CERTIFICATION_TITLE}
...
//# sourceMappingURL=AcademyPage.js.map