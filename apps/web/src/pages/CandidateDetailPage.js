...
import { getCandidate, updateCandidate, getCandidateActivities, STAGES, STAGE_LABELS, STAGE_COLORS } from '../services/recruitingService';
...
export default function CandidateDetailPage() {
    ...
    const advanceStage = async (newStage) => {
        ...
        setSaving(true);
        try {
            const updated = await updateCandidate(candidate.id, { stage: newStage });
    ...
    const saveScorecard = async () => {
        ...
        setSaving(true);
        try {
            const updated = await updateCandidate(candidate.id, { interview_scorecard: scorecard });
            setCandidate(updated);
        }
        catch (e) {
            console.error(e);
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {nextStages.map(stage => (<button key={stage} onClick={() => advanceStage(stage)} disabled={saving} className={`${STAGE_COLORS[stage]} hover:opacity-80 disabled:opacity-50 text-white text-xs px-3 py-1.5 rounded font-medium`}>
                  → {STAGE_LABELS[stage]}
                </button>))}
              {candidate.stage !== 'rejected' && (<button onClick={() => advanceStage('rejected')} disabled={saving} className="bg-red-700 hover:bg-red-600 disabled:opacity-50 text-white text-xs px-3 py-1.5 rounded font-medium">
                  ✕ Reject
                </button>)}
            </div>
            {nextStages.length === 0 && candidate.stage !== 'rejected' && candidate.stage !== 'active_tae' && (<p className="text-gray-500 text-xs mt-2">Final stage reached.</p>)}
          </div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
            ...
            <div className="space-y-3">
              {SCORECARD_FIELDS.map(({ key, label }) => (<div key={key} className="flex items-center gap-3">
                  <span className="text-gray-400 text-sm w-40">{label}</span>
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map(n => (<button key={n} onClick={() => setScorecard({ ...scorecard, [key]: n })} className={`w-8 h-8 rounded text-sm font-bold ${(scorecard[key] || 0) >= n
            ...
            <button onClick={saveScorecard} disabled={saving} className="mt-3 bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50 text-white text-xs px-3 py-1.5 rounded">
              Save Scorecard
            </button>
          </div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={4} className="w-full bg-gray-800 border border-gray-700 rounded p-3 text-white text-sm" placeholder="Interview notes, observations, follow-up items..."/>
            <button onClick={saveNotes} disabled={saving} className="mt-2 bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50 text-white text-xs px-3 py-1.5 rounded">
              Save Notes
            </button>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
            ...
            {candidate.resume_url ? (<div>
              ...
              </div>) : (<p className="text-gray-500 text-sm mb-2">No resume uploaded.</p>)}
            <label className="block mt-2">
              <input type="file" accept=".pdf,.doc,.docx" onChange={handleFileUpload} disabled={uploading} className="text-sm text-gray-300 file:mr-3 file:py-1 file:px-3 file:rounded file:border-0 file:text-sm file:bg-cyan-700 file:text-white hover:file:bg-cyan-600 disabled:opacity-50"/>
            </label>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
            ...
            <div className="space-y-3">
              {activities.map(a => (<div key={a.id} className="border-l-2 border-gray-700 pl-3">
...
//# sourceMappingURL=CandidateDetailPage.js.map