...
import { canSeeOrderValue, getOrderDueDate, getOrderTitle, matchesOrderQueueFilter, } from '../services/orderWorkflow';
const PAGE_SIZE = 10;
const filters = [
...
function StatusBadge({ stage }) {
    ...
    return (<span className={`inline-flex items-center rounded border px-2.5 py-0.5 text-xs font-bold uppercase tracking-wider ${cfg.className}`}>
      {cfg.label}
    </span>);
}
    ...
    const handleCreateOrder = async () => {
        try {
            ...
            if (!sourceOpportunity)
                throw new Error('No closed opportunities available');
    ...
    return (<div className="space-y-3 min-w-0">
      <Card title="Orders" className="min-w-0">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-2">
          {filters.map((filter) => (<button key={filter.key} onClick={() => { setQueueFilter(filter.key); setPage(1); }} className={`rounded-lg border px-3 py-1.5 text-xs font-bold transition ${queueFilter === filter.key
          ...
          <button onClick={() => { setSearch(''); setQueueFilter('ALL'); setPage(1); }} className="text-xs font-semibold text-slate-400 hover:text-slate-200">
            Reset
          </button>
        </div>
        ...
        {message ? (<p className="mb-4 rounded-lg border border-slate-600/40 bg-slate-800/40 px-3 py-2 text-sm text-slate-200">{message}</p>) : null}
        ...
        {/* Table */}
        {paged.length ? (<div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-800/50">
                ...
                return (<tr key={order.id} onClick={() => navigate(`/orders/${order.id}`)} className="cursor-pointer transition-colors hover:bg-slate-800/30">
                      ...
                      <td className="py-3 pr-3 text-right">
                        {canShowValue ? (<span className="font-semibold text-slate-200">{formatCurrency(order.value)}</span>) : (<span className="text-slate-600">—</span>)}
                      </td>
                      <td className="py-3 pr-3">
          ...
          </div>) : (<EmptyState title={allOrders.length ? 'No orders match this filter' : 'No orders yet'} description={allOrders.length ? 'Try switching to All Orders.' : 'Create an order from a closed-won opportunity to get started.'}/>)}
