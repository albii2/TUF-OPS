            ...
            <div className="space-y-2 text-xs">
              {opsWorkspaceQueue[section].map((order: any) => (
                <button key={order.id} onClick={() => navigate(`/orders/${order.id}`)} className="w-full rounded-lg panel-elevated p-2 text-left hover:bg-[#132133]">
                  ...
                  <p className="text-[10px] text-[var(--text-secondary)]">Lane: {order.lane} · Vendor: {order.vendor}</p><p className="text-[10px] text-[var(--text-secondary)]">{order.missingInfo.length ? `Blocker: ${order.missingInfo.join(', ')}` : 'Next: continue production handoff'}</p>
                </button>
              ))}
            </div>
          )}
