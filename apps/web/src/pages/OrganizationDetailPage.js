...
import { createEcosystemReferral, referredOrganizationTypes, warmIntroductionStatuses } from '../services/ecosystemReferralsService';
...
export function OrganizationDetailPage() {
    ...
    const laneCoverageBySport = Array.from(new Set(allOpportunities.filter((o) => o.organizationId === id).map((o) => o.sport))).map((sport) => {
        ...
        const has = (lane) => sportOpps.some((o) => o.lanes.includes(lane));
        return { sport, UNIFORM: has('UNIFORM'), TEAM_STORE: has('TEAM_STORE'), TRAVEL_GEAR: has('TRAVEL_GEAR'), LETTERMAN: has('LETTERMAN') };
    });
    if (!org)
        return <EmptyState title="Organization not found" description="Check the selected account and try again."/>;
    ...
    const updateReferralForm = (field, value) => {
        setReferralForm((current) => ({ ...current, [field]: value }));
        setReferralMessage('');
    };
    const attackLane = async (lane) => {
        if (!id || !org)
            return;
    ...
    return (<div className="space-y-3 min-w-0">
      <Card title="Account Penetration Console">
        <div className="flex items-center justify-between">
          <div>
            ...
            {(user && (user.role === 'DIRECTOR' || user.role === 'ADMIN')) && (<div className="mt-2 flex items-center gap-2">
                <span className="text-xs text-slate-500">Reassign Rep:</span>
                <select className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-xs text-slate-300" value={org.assignedRep} onChange={(e) => {
                ...
                window.dispatchEvent(new Event('tuf:org-updated'));
                  ...
                  {['Josh Hoffman', 'Primeau Hill', 'William Denzer', 'Ryan Streetar'].map((rep) => (<option key={rep} value={rep}>{rep}</option>))}
                </select>
              </div>)}
          </div>
      ...
      <Card title="Ecosystem Referrals from This Organization">
        {orgReferrals.length === 0 ? <p className="text-sm text-slate-400">No ecosystem referrals captured yet.</p> : (<div className="space-y-2 text-sm">
            {orgReferrals.map((referral) => (<div key={referral.id} className="rounded-lg border border-slate-800 bg-slate-950/60 p-3">
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        <Card title="Open Deals Driving Revenue" className="lg:col-span-2">
          {activeOpportunities.length === 0 ? <p className="text-sm text-slate-400">No active opportunities.</p> : (<div className="space-y-2 text-sm">
              {activeOpportunities.map((opp) => (<Link key={opp.id} className="block rounded-lg border border-slate-800 bg-slate-950/60 p-3 hover:border-cyan-400/40" to={`/opportunities/${opp.id}`}>
      ...
      <Card title="Activity & Relationship Signals">
        <div className="space-y-2 text-sm">
          {orgActivity.length ? orgActivity.map((entry) => (<div key={entry.id} className="rounded-lg border border-slate-800 bg-slate-950/60 p-3">
              <p>{entry.message}</p>
              <p className="text-xs text-slate-400">{entry.timestamp} · {entry.user}</p>
            </div>)) : <p className="text-slate-400">No account activity yet.</p>}
        </div>
      </Card>
    </div>);
}
//# sourceMappingURL=OrganizationDetailPage.js.map