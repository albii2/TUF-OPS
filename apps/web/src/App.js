...
import { OpportunitiesPage, OpportunityDetailPage, OpportunityNewPage, OrderDetailPage, OrdersPage, OrganizationDetailPage, OrganizationNewPage, OrganizationsPage, OpsWorkspacePage, ReportsPage, SettingsPage, TerritoryPage, TerritoryMapView, MyOpportunitiesPage, TeamOpportunitiesPage, TeamPerformancePage, EarningsPage, UsersPage, EcosystemPipelinePage, ChangeCredentialPage, } from './pages/CrudPages';
...
function RoleProtected({ user, allowedRoles, children }) {
    if (!user)
        return <Navigate to="/login" replace/>;
    if (user.mustChangeCredential)
        return <Navigate to="/change-credential" replace/>;
    if (!allowedRoles.includes(user.role))
        return <Navigate to="/dashboard" replace/>;
    return children;
}
function PageProtected({ user, path, children }) {
    if (!user)
        return <Navigate to="/login" replace/>;
    if (user.mustChangeCredential && path !== '/change-credential')
        return <Navigate to="/change-credential" replace/>;
    if (!roleConfig[user.role].visiblePages.includes(path) && path !== '/academy' && path !== '/admin/certification')
        return <Navigate to="/dashboard" replace/>;
    return children;
}
/**
 ...
 */
function CertificationProtected({ user, path, children }) {
    if (!user)
        return <Navigate to="/login" replace/>;
    if (user.mustChangeCredential && path !== '/change-credential')
        return <Navigate to="/change-credential" replace/>;
    ...
    if (UNCERTIFIED_ACCESSIBLE_PATHS.has(path))
        return children;
...
export default function App() {
    ...
    return (<Routes>
      ...
      <Route element={<Protected user={user}>
        ...
        <Route path="/ops-workspace" element={<RoleProtected user={user} allowedRoles={['ADMIN', 'REGIONAL_DIRECTOR']}>
              <OpsWorkspacePage />
            </RoleProtected>}/>
