...
const DEFAULT_LEAGUES = ['NBA', 'NFL', 'MLB', 'NHL'];
    ...
    try {
        ...
        if (raw) {
            ...
            if (prefs?.favoriteLeagues && Array.isArray(prefs.favoriteLeagues)) {
                const valid = prefs.favoriteLeagues.filter((l) => l in ESPN_SCOREBOARD_URLS);
                if (valid.length > 0)
                    return valid;
            }
        }
...
function injectMarqueeStyles() {
    ...
    .tuf-marquee-track {
      animation: tuf-marquee-scroll 180s linear infinite;
    }
...
export function SportsTicker() {
    const [scoreSlots, setScoreSlots] = useState([]);
    ...
    useEffect(() => {
        ...
        const loadScores = async () => {
            const results = await Promise.all(selectedLeagues.map(async (league) => {
                try {
                    const resp = await fetch(ESPN_SCOREBOARD_URLS[league]);
                    if (!resp.ok)
                        throw new Error(`ESPN ${league} status ${resp.status}`);
                    const payload = (await resp.json());
                    const events = payload.events ?? [];
                    // Map events to game slots
                    const gameSlots = events.map((event) => {
                        ...
                        const mnTeam = isMNTeam(event.name ?? '') ||
                            (comp?.competitors ?? []).some((c) => isMNTeam(c.team?.displayName ?? '') ||
            ...
            if (!cancelled) {
                ...
                setLastUpdated(`${displayHours}:${now.getMinutes().toString().padStart(2, '0')} ${ampm} CST`);
            }
        };
        loadScores();
    ...
    const marqueeContent = useMemo(() => {
        ...
        return doubled.map((item, index) => {
            if (item.type === 'wire') {
                return (<span key={`wire-${item.label}-${index}`} className="inline-flex items-center gap-2 whitespace-nowrap">
            ...
            // Game item
            return (<span key={`game-${item.leagueLabel}-${item.matchup}-${index}`} className="inline-flex items-center gap-2 whitespace-nowrap">
          {/* League badge */}
          ...
          {item.isLive && (<span className="tuf-live-dot inline-block h-2 w-2 rounded-full bg-emerald-400" title="Live"/>)}
          {item.isFinal && (<span className="inline-block h-2 w-2 rounded-full bg-red-500" title="Final"/>)}

          {/* MN Badge */}
          {item.isMNTeam && (<span className="rounded border border-purple-500/40 bg-purple-500/15 px-1 py-0.5 text-[9px] font-bold text-purple-300">
              MN
            </span>)}
          ...
          {/* Status / Game time */}
          {item.gameTime ? (<span className="text-slate-400">{item.gameTime}</span>) : null}
          <span className={`text-[11px] ${item.isLive
    ...
    return (<div className="mb-2 w-full min-w-0 overflow-hidden rounded-md border border-[#12314a] bg-[#07111b] text-xs text-slate-200">
      <div className="flex items-center">
        {/* Left fixed elements */}
        <a href="https://tufsports.us" target="_blank" rel="noopener noreferrer" className="shrink-0 border-r border-[#12314a] bg-[#0b1a28] px-3 py-2 font-semibold text-cyan-200 hover:bg-[#0f2a41] z-10">
          TUF Sports
        </a>
...
//# sourceMappingURL=SportsTicker.js.map