...
export default function TrainingModuleDetail({ module, enrollment, progress, onClose, onProgressUpdate, }) {
    ...
    const handleComplete = async () => {
        try {
            if (quizQuestions.length && !quizPassed) {
                ...
                if (result.passed) {
                    try {
                        localStorage.setItem(quizPassKey, 'true');
                    }
                    catch { }
                }
                if (!result.passed)
    ...
    return (<div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-2xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex items-start justify-between p-6 border-b border-slate-800/80">
          ...
          <button onClick={onClose} className="text-slate-500 hover:text-white text-2xl transition-colors">
            ✕
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {quizQuestions.length ? (<div className="mt-6 rounded-xl border border-cyan-400/20 bg-cyan-500/10 p-4">
              ...
              <div className="mt-3 space-y-4">
                {quizQuestions.map((question, index) => (<div key={question.question} className="rounded-lg border border-slate-800 bg-slate-950/70 p-3">
                    <p className="text-sm font-bold text-white">{index + 1}. {question.question}</p>
                    <div className="mt-2 grid gap-2">
                      {question.options.map((option) => (<label key={option} className="flex items-center gap-2 rounded-md border border-slate-800 bg-slate-900/50 px-3 py-2 text-sm text-slate-300">
                          <input type="radio" name={`quiz-${module.id}-${index}`} checked={quizAnswers[index] === option} onChange={() => setQuizAnswers((answers) => { const next = [...answers]; next[index] = option; return next; })}/>
                          <span>{option}</span>
                        </label>))}
                    </div>
                  </div>))}
        ...
        <div className="border-t border-slate-800/80 p-6 bg-slate-950/40">
          <div className="flex justify-between items-center mb-4">
            ...
            {module.estimated_duration_minutes && (<div>
          ...
          {/* Progress Bar */}
          {module.estimated_duration_minutes && (<div className="mb-6">
              <div className="w-full bg-slate-850 rounded-full h-2 overflow-hidden">
                <div className="bg-cyan-500 h-2 rounded-full transition-all shadow-[0_0_8px_rgba(6,182,212,0.5)]" style={{
          ...
          <div className="flex gap-3">
            <button onClick={onClose} className="flex-1 px-4 py-2.5 border border-slate-800 rounded-lg font-bold text-sm text-slate-300 hover:bg-slate-800 transition-colors">
              Close
            </button>
            {progress?.status === 'COMPLETED' && !completedWithoutRequiredQuiz ? (<button disabled className="flex-1 px-4 py-2.5 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 rounded-lg font-bold text-sm cursor-default">
                ✓ Completed
              </button>) : progress?.status === 'NOT_STARTED' || !progress ? (<button onClick={handleStart} disabled={loading} className="flex-1 px-4 py-2.5 bg-cyan-500 text-slate-950 hover:bg-cyan-400 rounded-lg font-bold text-sm disabled:opacity-50 transition-colors shadow-[0_0_12px_rgba(6,182,212,0.3)]">
                {loading ? 'Starting...' : 'Start Module'}
              </button>) : (<button onClick={handleComplete} disabled={loading} className="flex-1 px-4 py-2.5 bg-emerald-500 text-slate-950 hover:bg-emerald-400 rounded-lg font-bold text-sm disabled:opacity-50 transition-colors shadow-[0_0_12px_rgba(16,185,129,0.3)]">
...
//# sourceMappingURL=TrainingModuleDetail.js.map