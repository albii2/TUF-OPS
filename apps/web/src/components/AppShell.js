...
export function AppShell({ user, setUser }) {
    ...
    return (<div className="relative min-h-screen bg-tuf-texture text-[var(--text-primary)]">
      <div className="mx-auto grid min-h-screen max-w-[1200px] grid-cols-1 md:grid-cols-[240px_minmax(0,1fr)]">
        <aside className="hidden border-r border-[var(--border)] bg-[#070c13]/95 p-3.5 md:flex md:flex-col">
          ...
          <nav className="mt-3.5 space-y-1 flex-1">
            ...
            return (<NavLink key={nav.key} to={nav.route} className={({ isActive }) => `block rounded-md border px-3 py-1.5 text-sm ${isActive ? 'glow-blue border-[#1FB6FF] bg-[#0d2234] text-[#dff5ff]' : 'border-transparent text-[var(--text-secondary)] hover:border-[var(--border)] hover:bg-[#0d1723]'}`}>
                  {nav.label}
                </NavLink>);
        })}
          </nav>
        ...
        <main className="min-w-0 px-4 pb-24 pt-3 md:px-5 md:pb-20">
          <div className="mb-3 md:hidden">
            ...
            <nav className="flex gap-2 overflow-x-auto pb-1">
              ...
              {navItems.filter((nav) => nav.key !== 'academy').map((nav) => (<NavLink key={nav.key} to={nav.route} className={({ isActive }) => `shrink-0 rounded-md border px-3 py-2 text-xs ${isActive ? 'border-[#1FB6FF] bg-[#0d2234] text-[#dff5ff]' : 'border-[var(--border)] bg-[#0b1118] text-[var(--text-secondary)]'}`}>
                  {nav.label}
                </NavLink>))}
            </nav>
          </div>
          ...
          <header className="mb-3 rounded-lg panel p-2">
            <SportsTicker />
            {user.role === 'DIRECTOR' || user.role === 'REP' ? (<div className="mb-2 flex flex-wrap gap-2 text-[11px] font-semibold uppercase tracking-wide">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              <div className="flex-1">
                <input className="h-9 w-full rounded-md panel-elevated px-3 text-sm text-[var(--text-primary)] placeholder:text-[var(--text-secondary)]" placeholder="Search accounts, deals, orders..." value={search} onChange={(e) => {
            ...
            if (e.key === 'Enter')
              ...
              {user.role === 'ADMIN' ? (<select aria-label='Beta role context' className="h-9 rounded-md panel-elevated px-2 text-xs" value={user.role} onChange={(e) => { const next = updateRole(e.target.value); if (next) {
            setUser(next);
            navigate('/dashboard');
        } }}>
                  {['ADMIN', 'DIRECTOR', 'REP', 'OPERATIONS'].filter((role) => role === user.role || activeRoles.has(role)).map((role) => <option key={role} value={role}>{role}</option>)}
                </select>) : null}
