...
import { getForgeMission } from '../services/forgeEngine';
export const ForgePanel = () => {
    ...
    if (!mission || mission.topAccounts.length === 0) {
        return (<div className="bg-[#0a0a0f] border border-[#1e293b] rounded-lg p-4">
    ...
    return (<div className="bg-[#0a0a0f] border border-[#1e293b] rounded-lg overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 bg-[#111118] border-b border-[#1e293b] cursor-pointer" onClick={() => setCollapsed(!collapsed)}>
      ...
      {!collapsed && (<div className="p-4 space-y-4">
          ...
          <div>
            ...
            <div className="space-y-2">
              {mission.topAccounts.map((acct, i) => (<div key={acct.organizationId} className={`border rounded p-3 ${priorityBg(acct.priority)}`}>
                  <div className="flex items-start justify-between">
                    <div>
                      <span className={`text-sm font-semibold ${priorityColor(acct.priority)}`}>
                        {acct.organizationName}
                      </span>
                      {acct.opportunityStage && (<span className="ml-2 text-[10px] uppercase tracking-wider text-slate-500 bg-slate-800/50 px-1.5 py-0.5 rounded">
                  ...
                  {acct.daysSinceContact && acct.daysSinceContact > 7 && (<p className="text-rose-400/60 text-[10px] mt-1">{acct.daysSinceContact} days since last contact</p>)}
                </div>))}
            </div>
          </div>

          {/* Pipeline Gaps */}
          {mission.pipelineGaps.length > 0 && (<div>
              <h4 className="text-rose-400/80 text-xs font-mono tracking-wider mb-1.5">GAPS</h4>
              <ul className="space-y-1">
                {mission.pipelineGaps.map((g, i) => (<li key={i} className="text-slate-400 text-xs">• {g}</li>))}
              </ul>
            </div>)}

          {/* At-Risk */}
          {mission.atRiskOpportunities.length > 0 && (<div>
              <h4 className="text-amber-400/80 text-xs font-mono tracking-wider mb-1.5">AT RISK</h4>
              <div className="space-y-1.5">
                {mission.atRiskOpportunities.map((acct) => (<div key={acct.organizationId} className="bg-amber-400/5 border border-amber-400/20 rounded p-2">
