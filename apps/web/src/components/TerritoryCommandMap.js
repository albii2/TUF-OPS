...
const laneOrder = ['UNIFORM', 'TEAM_STORE', 'TRAVEL_GEAR', 'LETTERMAN'];
const zoneOrder = ['north', 'west', 'metro', 'south'];
const zoneTone = {
...
function ZoneButton({ zone, active, onSelect }) {
    const tone = zoneTone[zone.id];
    const coverage = percent(zone.accounts - zone.untouched, zone.accounts);
    return (<button aria-pressed={active} className={`${tone.grid} relative overflow-hidden rounded-xl border p-4 text-left transition hover:-translate-y-0.5 ${active ? 'border-cyan-200 ring-2 ring-cyan-300/80' : 'border-white/20'}`} onClick={onSelect} style={{ background: `linear-gradient(135deg, ${tone.color}ee, ${tone.color}55)` }}>
...
export function TerritoryCommandMap({ title = 'TERRITORY COMMAND MAP', fullMapLink = true }) {
    const { data: territories = [] } = useTerritories();
    const { data: organizations = [] } = useOrganizations({});
    const { data: opportunities = [] } = useOpportunities({});
    const [activeZone, setActiveZone] = useState('metro');
    ...
    const orderedZones = zoneOrder.map((id) => zoneMetrics.find((zone) => zone.id === id)).filter(Boolean);
    const active = zoneMetrics.find((zone) => zone.id === activeZone) ?? zoneMetrics[0];
    return (<GlassCard title={title}>
      ...
      <div className="grid gap-3 xl:grid-cols-[minmax(0,1.35fr)_minmax(300px,0.7fr)]">
        <div className="rounded-xl border border-slate-800 bg-[#07101a] p-3">
          <div className="grid min-h-[420px] grid-cols-6 grid-rows-5 gap-2">
            {orderedZones.map((zone) => (<ZoneButton key={zone.id} zone={zone} active={active?.id === zone.id} onSelect={() => setActiveZone(zone.id)}/>))}
          </div>
        </div>

        {active ? (<div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3">
            ...
            <div className="mt-4 space-y-2"><p className="text-xs text-slate-400">Top Accounts: {active.topAccounts.join(', ') || 'None'}</p><p className="text-xs text-slate-400">Recommended owner action: {active.untouched > 5 ? 'Assign untouched accounts and pressure first-call outreach.' : active.stuck > active.nearClose ? 'Coach stuck-deal reps and unblock decision path.' : 'Push near-close opportunities into invoice and handoff.'}</p>
              ...
              {laneOrder.map((lane) => (<div key={lane} className="space-y-1">
      ...
      <div className="mt-3 grid gap-2 md:grid-cols-4">
        {orderedZones.map((zone) => (<button key={zone.id} onClick={() => setActiveZone(zone.id)} className={`rounded-lg border p-3 text-left transition ${active?.id === zone.id ? 'border-cyan-300 bg-cyan-500/10' : 'border-slate-800 bg-slate-950/60 hover:border-slate-600'}`}>
...
//# sourceMappingURL=TerritoryCommandMap.js.map