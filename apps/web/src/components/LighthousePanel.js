...
import { getOrganizationIntel } from '../services/lighthouseEngine';
export const LighthousePanel = ({ organizationId, organizationName }) => {
    ...
    if (error)
        return <div className="bg-[#0a0a0f] border border-[#1e293b] rounded-lg p-4 mt-4"><p className="text-slate-500 text-sm">{error}</p></div>;
    if (!intel)
        return <div className="bg-[#0a0a0f] border border-[#1e293b] rounded-lg p-4 mt-4"><p className="text-slate-500 text-sm">Loading intelligence...</p></div>;
    const urg = (u) => u === 'NOW' ? 'text-rose-400 bg-rose-400/10' : u === 'SOON' ? 'text-amber-400 bg-amber-400/10' : 'text-slate-400 bg-slate-400/10';
    return (<div className="bg-[#0a0a0f] border border-[#1e293b] rounded-lg overflow-hidden mt-4">
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-[#111118] border border-[#1e293b] rounded p-3">
            ...
            {intel.sportsMapped.length > 0 ? <div className="flex flex-wrap gap-1">{intel.sportsMapped.map((s) => <span key={s} className="text-[10px] bg-blue-400/10 text-blue-400 px-2 py-0.5 rounded">{s}</span>)}</div> : <p className="text-slate-500 text-xs">None</p>}
          </div>
          <div className="bg-[#111118] border border-[#1e293b] rounded p-3">
            <h4 className="text-slate-400 text-[10px] font-mono tracking-wider mb-2">LANES ACTIVE</h4>
            {intel.revenueLanesActivated.length > 0 ? <div className="flex flex-wrap gap-1">{intel.revenueLanesActivated.map((l) => <span key={l} className="text-[10px] bg-amber-400/10 text-amber-400 px-2 py-0.5 rounded">{l}</span>)}</div> : <p className="text-slate-500 text-xs">None</p>}
          </div>
        </div>
        {intel.revenueLanesMissing.length > 0 && <div><h4 className="text-rose-400/80 text-xs font-mono tracking-wider mb-1.5">MISSING LANES</h4><div className="flex flex-wrap gap-1">{intel.revenueLanesMissing.map((l) => <span key={l} className="text-[10px] bg-rose-400/10 text-rose-400 px-2 py-0.5 rounded">{l}</span>)}</div></div>}
        <div className="bg-[#111118] border border-[#1e293b] rounded p-3">
