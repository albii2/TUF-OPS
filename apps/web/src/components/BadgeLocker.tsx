...
import type { RepStats, CollectionId, QualificationProgress } from '../lib/achievements';
import {
...
export function BadgeLocker({ stats }: BadgeLockerProps) {
  ...
  const [expandedCollection, setExpandedCollection] = useState<CollectionId | null>(null);
          ...
          return (
            <div key={collection.id}>
              ...
              <button
                ...
                className="flex w-full items-center gap-3 rounded border border-slate-800 bg-[#0a0a16] px-4 py-3 text-left transition-colors hover:border-slate-700"
              >
              ...
              {/* Expanded badge grid */}
              {isExpanded && (
                <div className="border-x border-b border-slate-800 bg-[#060610] px-4 py-4">
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
                    {progress.map((qp) => (
                      <div key={qp.qualification.id} className="space-y-2">
                        <QualificationBadge
                          qualification={qp.qualification}
                          earnedTier={qp.earnedTier}
                          progress={qp}
                          size="md"
                        />
                        ...
                        <div className="space-y-0.5">
                          {qp.tierProgress.map((tp) => {
                            ...
                            return (
                              <div key={tp.tier} className="flex items-center gap-1.5">
                                <span
                                  className={`text-[8px] font-bold uppercase tracking-widest w-10 text-right ${
                                    tp.unlocked
                                      ? tp.tier === 'black'
                                        ? 'text-amber-400'
                                        : 'text-slate-400'
                                ...
                                <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-slate-800">
                                  <div
                                    ...
                                    style={{ width: `${tp.unlocked ? 100 : isCurrentTier ? tp.percent : 0}%` }}
                                  />
                                </div>
                                <span
                                  className={`text-[8px] font-mono tabular-nums w-8 ${
                                    tp.unlocked ? 'text-slate-400' : 'text-slate-600'
                                  }`}
                                >
                        ...
                        {/* Earned date */}
                        {qp.earnedAt && (
                          <div className="text-center text-[7px] uppercase tracking-wider text-slate-500">
                            UNLOCKED{' '}
