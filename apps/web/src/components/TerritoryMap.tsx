...
interface SchoolRecord {
  ...
  football_offered: string;
  basketball_offered: string;
  hockey_offered: string;
  baseball_offered: string;
  address: string;
}

type ZoneKey = 'TUF Metro' | 'TUF South' | 'TUF West' | 'TUF North';
type TierKey = 'Tier 1' | 'Tier 2' | 'Tier 3';
...
const ZONE_ORDER: ZoneKey[] = ['TUF Metro', 'TUF South', 'TUF West', 'TUF North'];
...
const TIER_ORDER: TierKey[] = ['Tier 1', 'Tier 2', 'Tier 3'];

const MN_CENTER: [number, number] = [45.5, -94.0];
const DEFAULT_ZOOM = 7;

// ── SVG marker icons ───────────────────────────────────
function createMarkerIcon(color: string, tier?: TierKey): L.DivIcon {
...
function loadSchoolsSync(): SchoolRecord[] {
  const text = csvRaw;
...
function SearchBox({ onSelect }: { onSelect: (school: SchoolRecord) => void }) {
...
export function TerritoryMapView() {
  ...
  const [flyTo, setFlyTo] = useState<{ center: [number, number]; zoom: number }>({
    center: MN_CENTER,
    zoom: DEFAULT_ZOOM,
  });

  // Toggle filters
  const [zoneFilters, setZoneFilters] = useState<Set<ZoneKey>>(
    new Set(ZONE_ORDER)
  );
  const [tierFilters, setTierFilters] = useState<Set<TierKey>>(
    new Set(TIER_ORDER)
  );
  const [repFilters, setRepFilters] = useState<Set<string>>(new Set(['all']));
  const [searchSelected, setSearchSelected] = useState<SchoolRecord | null>(null);
  ...
  const filteredSchools = useMemo(() => {
    return schools.filter((s) => {
      const zone = s.tuf_zone as ZoneKey;
      const tier = s.tuf_priority as TierKey;

      if (!zoneFilters.has(zone)) return false;
  ...
  const toggleTier = useCallback((t: TierKey) => {
    setTierFilters((prev) => {
      const next = new Set(prev);
  ...
  const handleSearchSelect = useCallback((school: SchoolRecord) => {
        ...
        <div className="grid gap-3 lg:grid-cols-[1fr_280px]">
          ...
          >
            ...
            >
              ...
              {ZONE_ORDER.map((zone) => {
                if (!zoneFilters.has(zone)) return null;
                // Just a rough circle for each zone region
                const centers: Record<ZoneKey, [number, number]> = {
                  'TUF Metro': [44.97, -93.26],
                  'TUF South': [44.05, -93.5],
                  'TUF West': [45.55, -95.5],
                  'TUF North': [47.2, -93.0],
                };
                const radii: Record<ZoneKey, number> = {
                  'TUF Metro': 65000,
                  'TUF South': 180000,
                  'TUF West': 200000,
                  'TUF North': 250000,
              ...
              {filteredSchools.map((school, idx) => {
                const coords = getCoordsForZip(school.zip);
                const zone = school.tuf_zone as ZoneKey;
                const tier = school.tuf_priority as TierKey;
          ...
          <div className="space-y-3">
            ...
            <div className="panel-elevated rounded-lg p-3">
              ...
              {ZONE_ORDER.map((zone) => (
                <label
                  key={zone}
                  className="flex items-center gap-2 mb-1.5 cursor-pointer text-sm"
                  style={{ opacity: zoneFilters.has(zone) ? 1 : 0.4 }}
                >
                  <input
                    type="checkbox"
            ...
            <div className="panel-elevated rounded-lg p-3">
              ...
              {TIER_ORDER.map((tier) => {
                const count = schools.filter(
                  (s) => s.tuf_priority === tier
                ).length;
                return (
                  <label
                    key={tier}
                    className="flex items-center gap-2 mb-1.5 cursor-pointer text-sm"
                    style={{ opacity: tierFilters.has(tier) ? 1 : 0.4 }}
                  >
                    <input
                      type="checkbox"
          ...
          return (
            <button
              ...
              className={`rounded-lg border p-3 text-left transition-colors ${
                zoneFilters.has(zone) && zoneFilters.size === 1
