...
export default function LockerRoomSimulator() {
  ...
  const selectScenario = (scenario: LockerRoomScenario) => {
    setActive(scenario);
    setUserPitch('');
    setEvaluation(null);
  };
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[280px_1fr]">
        <div className="space-y-2">
          {LOCKER_ROOM_SCENARIOS.map((scenario) => (
            <button key={scenario.title} onClick={() => selectScenario(scenario)} className={`w-full rounded-lg border px-3 py-2 text-left text-sm transition ${active.title === scenario.title ? 'border-cyan-300 bg-cyan-500/15 text-white' : 'border-slate-800 bg-slate-900/60 text-slate-300 hover:border-cyan-500/50'}`}>{scenario.title}</button>
          ))}
        </div>
        <div className="rounded-xl border border-slate-800 bg-[#070c13]/80 p-4">
          ...
          <div className="mt-4 space-y-2">
            ...
            <textarea
              ...
              placeholder={`Write how you would respond. E.g. ${active.suggestedOpening}`}
              rows={4}
              className="w-full rounded-lg border border-slate-800 bg-slate-950 p-3 text-sm text-slate-100 placeholder-slate-700 focus:border-cyan-500 focus:outline-none focus:ring-1 focus:ring-cyan-500/30"
            />
          </div>
          ...
          <div className="mt-4 flex gap-3">
            <button
              onClick={submitPitch}
              disabled={submitting || !userPitch.trim()}
              className="rounded-lg bg-cyan-500 hover:bg-cyan-400 px-4 py-2.5 text-xs font-bold text-slate-950 disabled:opacity-40 transition-colors shadow-[0_0_12px_rgba(6,182,212,0.2)]"
            >
              {submitting ? 'Evaluating...' : 'Submit Pitch for LLM Review'}
            </button>
            <button
              onClick={retryScenario}
              className="rounded-lg border border-slate-800 hover:border-slate-700 px-4 py-2.5 text-xs font-bold text-slate-300 transition-colors"
            >
              Clear & Reset
            </button>
          </div>
          ...
          {evaluation && (
            <div className={`mt-4 rounded-xl border p-4 transition-all ${evaluation.passed ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-rose-500/30 bg-rose-500/5'}`}>
