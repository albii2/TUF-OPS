...
export default function CrmWalkthroughTour() {
    ...
    if (path.match(/^\/opportunities\/[^/]+$/)) {
        ...
        if (id) {
            try {
                const localOpps = JSON.parse(localStorage.getItem('tuf_ops_opportunities_v2') || '[]');
    ...
    if (!active) {
        ...
        <button onClick={startTour} className="glow-blue flex items-center gap-2 rounded-full border border-cyan-400/40 bg-slate-950/90 px-4 py-2.5 text-xs font-black uppercase tracking-wider text-cyan-300 shadow-xl backdrop-blur transition hover:scale-105 active:scale-95">
    ...
    return (<div className="fixed bottom-6 right-6 z-50 w-80 rounded-2xl border border-cyan-400/45 bg-[#050b12]/95 p-4 shadow-2xl shadow-cyan-950/40 backdrop-blur-md">
      ...
      <div className="mt-4 flex items-center justify-between gap-3 border-t border-slate-800/40 pt-3">
        <div className="flex gap-1">
          {steps.map((_, idx) => (<div key={idx} className={`h-1.5 w-3 rounded-full transition-all ${idx + 1 === step
        ...
        <div className="flex gap-2">
          {step > 1 && (<button onClick={() => setStep((prev) => prev - 1)} className="rounded bg-slate-800 px-2 py-1 text-[10px] font-bold text-slate-300 hover:bg-slate-700">
              Back
            </button>)}
          {step < 7 ? (<button onClick={() => setStep((prev) => prev + 1)} className="rounded bg-cyan-500/20 border border-cyan-400/50 px-2 py-1 text-[10px] font-bold text-cyan-100 hover:bg-cyan-500/30">
              Skip
            </button>) : (<button onClick={endTour} className="rounded bg-emerald-500/25 border border-emerald-400/50 px-2.5 py-1 text-[10px] font-black text-emerald-100 hover:bg-emerald-500/40 shadow-lg shadow-emerald-950/30">
              Finish Tour
            </button>)}
        </div>
      </div>
...
//# sourceMappingURL=CrmWalkthroughTour.js.map