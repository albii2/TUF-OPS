...
export default function TrainingModuleCard({ module, progress, onSelect }: TrainingModuleCardProps) {
  ...
  return (
    <div
      onClick={onSelect}
      className="p-6 bg-[#070c13]/60 rounded-xl border border-slate-800/80 hover:border-cyan-500/50 hover:shadow-[0_0_15px_rgba(6,182,212,0.08)] transition-all cursor-pointer flex flex-col justify-between h-full"
    >
      <div>
        {/* Header */}
        ...
        {/* Description */}
        {module.description && (
          <p className="text-xs text-slate-400 mb-4 line-clamp-2 leading-relaxed">{module.description}</p>
        )}
      </div>
      ...
      <div>
        ...
        <div className="flex items-center gap-4 text-xs text-slate-500 mb-4">
          ...
          {module.estimated_duration_minutes && (
            <span className="flex items-center gap-1">⏱️ {module.estimated_duration_minutes} min</span>
          )}
        </div>

        {/* Progress Bar */}
        {isStarted && progress?.time_spent_seconds !== undefined && (
          <div className="mb-4">
            ...
            <div className="w-full bg-slate-800 rounded-full h-1.5 overflow-hidden">
              <div
                className="bg-cyan-400 h-full rounded-full transition-all shadow-[0_0_8px_rgba(6,182,212,0.5)]"
                style={{
                  width: `${Math.min(
        ...
        {/* CTA Button */}
        <button className={`w-full px-4 py-2.5 rounded-lg font-bold text-xs transition-all border ${
          isCompleted 
