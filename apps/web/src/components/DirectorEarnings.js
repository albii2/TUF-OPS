...
function getDirectorTeamSummary(directorName) {
    const director = teamMembers.find((member) => member.name === directorName);
    if (!director)
        return { teamRows: [], teamRevenue: 0, teamOrders: 0, teamPipeline: 0 };
...
export function DirectorEarnings({ directorName }) {
    ...
    return (<div className="space-y-3">
      ...
      <Card title="Rep Pace">
        <div className="space-y-2">
          {teamRows.map((row) => (<div key={row.repName} className="grid gap-2 rounded border border-slate-800 p-2 text-sm md:grid-cols-4">
...
//# sourceMappingURL=DirectorEarnings.js.map