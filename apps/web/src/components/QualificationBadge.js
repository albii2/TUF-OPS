...
function ShapeSVG({ shape, accent, earned, size }) {
    ...
    switch (shape) {
        case 'shield':
            return (<svg width={dims} height={dims} viewBox="0 0 40 40" fill="none">
          <path d="M20 2L36 10V20C36 30 28 37 20 38C12 37 4 30 4 20V10L20 2Z" fill={fillColor} stroke={strokeColor} strokeWidth="1.5"/>
        </svg>);
        case 'chevron':
            return (<svg width={dims} height={dims} viewBox="0 0 40 40" fill="none">
          <path d="M20 4L36 20L20 36L4 20L20 4Z" fill={fillColor} stroke={strokeColor} strokeWidth="1.5" transform="rotate(45 20 20)"/>
        </svg>);
        case 'hexagon':
            return (<svg width={dims} height={dims} viewBox="0 0 40 40" fill="none">
          <polygon points="20,2 37,11 37,29 20,38 3,29 3,11" fill={fillColor} stroke={strokeColor} strokeWidth="1.5"/>
        </svg>);
        case 'compass':
            return (<svg width={dims} height={dims} viewBox="0 0 40 40" fill="none">
        ...
        case 'diamond':
            return (<svg width={dims} height={dims} viewBox="0 0 40 40" fill="none">
          <polygon points="20,3 37,20 20,37 3,20" fill={fillColor} stroke={strokeColor} strokeWidth="1.5"/>
        </svg>);
        case 'star':
            return (<svg width={dims} height={dims} viewBox="0 0 40 40" fill="none">
          <polygon points="20,3 24,15 37,15 27,23 30,36 20,28 10,36 13,23 3,15 16,15" fill={fillColor} stroke={strokeColor} strokeWidth="1.5"/>
        </svg>);
        default:
            return (<svg width={dims} height={dims} viewBox="0 0 40 40" fill="none">
...
export function QualificationBadge({ qualification, earnedTier, progress, size = 'md', }) {
    ...
    const progressPct = progress
        ? Math.round((progress.tierProgress.filter((t) => t.unlocked).length / progress.tierProgress.length) * 100)
        : 0;
    return (<div className={`relative flex flex-col items-center rounded-lg border p-3 transition-all duration-300 ${earned
            ? isBlack
                ? 'border-[#1a1a2e] bg-[#0a0a16]'
                : `${metal?.border ?? ''} ${metal?.bg ?? ''}`
            : LOCKED_STYLE} ${sizeClasses[size]}`}>
      {/* Light sweep animation on newly earned */}
      {earned && (<div className="pointer-events-none absolute inset-0 overflow-hidden rounded-lg">
          <div className="absolute inset-0 animate-light-sweep opacity-0" style={{
      ...
      <span className={`text-center text-[10px] font-bold uppercase tracking-[0.15em] leading-tight ${earned ? (isBlack ? 'text-amber-400' : 'text-slate-200') : 'text-slate-600'}`}>
        {qualification.name}
      </span>

      {/* Tier label */}
      <span className={`mt-1 rounded border px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-widest ${earned
            ? isBlack
      ...
      {/* Collection label */}
      <span className="mt-1 text-[7px] font-medium uppercase tracking-[0.2em] text-slate-500">
        {collection.label}
      </span>

      {/* Tier progress dots */}
      {size !== 'sm' && (<div className="mt-2 flex gap-1">
          {['bronze', 'silver', 'gold', 'black'].map((t) => {
                ...
                return (<div key={t} className={`h-1.5 w-1.5 rounded-full border ${isUnlocked
                        ? t === 'black'
...
//# sourceMappingURL=QualificationBadge.js.map